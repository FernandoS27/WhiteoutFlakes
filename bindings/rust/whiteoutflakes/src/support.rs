...
//
// Hand-written runtime support shared by the generated module. Mirrors
// WhiteoutLib's `bindings/rust/whiteout/src/support.rs` — the two C ABIs
// are emitted by the same generator and share the `whiteout_Bytes` /
// `whiteout_CString` wire types.
//
// Some items here are ahead of what the renderer surface currently
// returns; they are kept so a regenerated module that starts producing
// bytes or borrowed views compiles without a matching edit here.
...
///
/// `owner` is the ownership discriminator: non-null means the C++ side
...
impl Bytes {
    ...
    #[allow(dead_code)] // used by generated bytes-returning methods
...
///
/// The lifetime is tied to a borrow of the owning handle, so the compiler
/// rejects any use after the owner is dropped or mutably re-borrowed.
...
impl BorrowedSlice<'_> {
    /// # Safety
    /// `ptr`/`len` must describe a buffer that stays valid and immutable
    /// for `'a`.
    #[allow(dead_code)] // used by generated span-returning methods
