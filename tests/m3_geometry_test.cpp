    ...
    for (const char* name : kCorpora) {
        ...
        for (const auto& path : models) {
            ...
            for (const auto& m : meshes) {
                ...
                // The upload path builds a fully interleaved vertex and reads
                // all three arrays, so they must agree in length even though
                // only positions carry meaning at this stage.
