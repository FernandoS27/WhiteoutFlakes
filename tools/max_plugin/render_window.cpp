...
void RenderWindow::ThreadFunc(i32 w, i32 h, gfx::GfxApi api) {
    ...
    while (running_) {
        ...
        if (service_.Replaceables().ConsumeDirty())
            InvalidateTeamColorSwatch();
...
bool RenderWindow::Create(i32 w, i32 h) {
    ...
    hMenuTileset_ = CreatePopupMenu();
    {
        ...
        for (i32 i = 0; i < n; ++i) {
            const char* nm =
                whiteout::flakes::io::TilesetName(static_cast<whiteout::flakes::io::Tileset>(i));
            wchar_t wbuf[64];
    ...
    AppendMenuW(hMenuView_, MF_POPUP | MF_STRING, (UINT_PTR)hMenuTileset_, L"Tileset");
    ...
    CheckMenuRadioItem(hMenuDebugVis_, IDM_DBGVIS_BASE, IDM_DBGVIS_BASE + 7,
                       IDM_DBGVIS_BASE + (initDbg >= 0 && initDbg < 8 ? initDbg : 0), MF_BYCOMMAND);
    AppendMenuW(hMenuDebug_, MF_POPUP | MF_STRING, (UINT_PTR)hMenuDebugVis_, L"Debug View");

    static const wchar_t* const kLodLabels[5] = {
        L"Auto (screen size)", L"Force LOD 0 (base)",   L"Force LOD 1",
        L"Force LOD 2",        L"Force LOD 3 (lowest)",
    };
    for (i32 i = 0; i < 5; ++i)
    ...
    CheckMenuRadioItem(hMenuLod_, IDM_LOD_BASE, IDM_LOD_LAST, IDM_LOD_BASE + lodCheckIdx,
                       MF_BYCOMMAND);
    ...
    hwnd_ = CreateWindowExW(0, WINDOW_CLASS, WINDOW_TITLE, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
                            ...
                            hMenuBar_, hInst, this);
    if (!hwnd_)
        return false;

    hwndRender_ = CreateWindowExW(0, RENDER_CLASS, L"", WS_CHILD | WS_VISIBLE, 0, kToolbarH, w, h,
                                  hwnd_, nullptr, hInst, this);
    if (!hwndRender_)
        return false;

    i32 x = 8;
    lblSequence_ = CreateWindowW(L"STATIC", L"Animation:", WS_CHILD | SS_CENTERIMAGE, x, 4, 64, 20,
                                 hwnd_, nullptr, hInst, nullptr);
    x += 66;
    cmbSequence_ = CreateWindowW(L"COMBOBOX", L"", WS_CHILD | CBS_DROPDOWNLIST | WS_VSCROLL, x, 2,
                                 180, 300, hwnd_, (HMENU)(INT_PTR)IDC_SEQUENCE, hInst, nullptr);
    x += 188;

    CreateWindowW(L"STATIC", L"Camera:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, x, 4, 50, 20,
                  hwnd_, nullptr, hInst, nullptr);
    x += 52;

    cmbCamera_ =
        CreateWindowW(L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, x, 2,
                      110, 200, hwnd_, (HMENU)(INT_PTR)IDC_CAMERA, hInst, nullptr);
    SendMessageW(cmbCamera_, CB_ADDSTRING, 0, (LPARAM)L"Free Camera");
    SendMessageW(cmbCamera_, CB_SETCURSEL, 0, 0);
    x += 118;

    CreateWindowW(L"STATIC", L"Team:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, x, 4, 36, 20, hwnd_,
                  nullptr, hInst, nullptr);
    x += 38;
    btnTeamColor_ = CreateWindowW(L"BUTTON", L"", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, x, 4, 22,
                                  20, hwnd_, (HMENU)(INT_PTR)IDC_TEAMCOLOR, hInst, nullptr);
    x += 30;

    CreateWindowW(L"STATIC", L"Lighting:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, x, 4, 56, 20,
                  hwnd_, nullptr, hInst, nullptr);
    x += 58;
    cmbLighting_ =
        CreateWindowW(L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, x, 2,
                      100, 200, hwnd_, (HMENU)(INT_PTR)IDC_LIGHTING, hInst, nullptr);
...
LRESULT RenderWindow::HandleMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE: {
        ...
        if (w > 0 && renderH > 0) {
            if (hwndRender_)
                MoveWindow(hwndRender_, 0, kToolbarH, w, renderH, TRUE);
            if (service_.Pipeline().IsDeviceReady())
                service_.Pipeline().ResizePrimaryTarget(w, renderH);
        }
        return 0;
    }
    case WM_LBUTTONDOWN: {
    ...
    case WM_MOUSEMOVE: {
        ...
        service_.Debug().SetViewCubeHovered(cur.x >= vcr.left && cur.x <= vcr.right &&
    ...
    case WM_DRAWITEM: {
        ...
        if (dis->CtlID == IDC_TEAMCOLOR) {
            auto* focus =
                service_.Scene().Actors().Find(focusActor_.load(std::memory_order_relaxed));
            COLORREF tc = focus ? (COLORREF)(focus->teamColor & 0x00FFFFFFu) : RGB(255, 0, 0);
    ...
    case WM_COMMAND: {
        ...
        if (id == IDM_VIEW_GRID) {
            toggleView(id, &DisplayFlags::showGrid);
            return 0;
        }
        if (id == IDM_VIEW_PARTICLES) {
            toggleView(id, &DisplayFlags::showParticles);
            return 0;
        }
        if (id == IDM_VIEW_RIBBONS) {
            toggleView(id, &DisplayFlags::showRibbons);
            return 0;
        }
        if (id == IDM_VIEW_EVENTS) {
            toggleView(id, &DisplayFlags::showEvents);
            return 0;
        }
        if (id == IDM_DBG_COLLISIONS) {
            toggleView(id, &DisplayFlags::showCollisions);
            return 0;
        }
        if (id == IDM_DBG_LIGHTS) {
            toggleView(id, &DisplayFlags::showLights);
            return 0;
        }
        ...
        if (id >= (i32)IDM_TILESET_BASE && id <= (i32)IDM_TILESET_LAST) {
            ...
            if (idx < 0 || idx >= n)
                return 0;
            CheckMenuRadioItem(hMenuTileset_, IDM_TILESET_BASE, IDM_TILESET_LAST, id, MF_BYCOMMAND);
        ...
        case IDC_TEAMCOLOR: {
            ...
            if (ChooseColorW(&cc)) {
                focus->SetTeamColor(GetRValue(cc.rgbResult), GetGValue(cc.rgbResult),
        ...
        case IDC_SEQUENCE: {
            if (code == CBN_SELCHANGE) {
                ...
                if (focus)
                    focus->animation.SetActiveSequenceIndex(sel);
                if (sel == prev)
                    break;
...
void RenderWindow::SyncViewMenuFromService() {
    ...
    auto syncToggle = [&](UINT id, bool checked) {
        CheckMenuItem(hMenuBar_, id, MF_BYCOMMAND | (checked ? MF_CHECKED : MF_UNCHECKED));
    };
    ...
    if (hMenuTileset_) {
        const i32 n = static_cast<i32>(whiteout::flakes::io::Tileset::Count);
        const i32 curIdx =
            std::clamp(static_cast<i32>(whiteout::flakes::io::GetCurrentTileset()), 0, n - 1);
        CheckMenuRadioItem(hMenuTileset_, IDM_TILESET_BASE, IDM_TILESET_LAST,
                           IDM_TILESET_BASE + curIdx, MF_BYCOMMAND);
    }
...
void RenderWindow::EnsureSettingsWindow() {
    ...
    hwndSettings_ =
        CreateWindowExW(exStyle, SETTINGS_CLASS, L"Settings", style, x, y, rc.right - rc.left,
                        rc.bottom - rc.top, hwnd_, nullptr, hInst, this);
    if (!hwndSettings_)
        return;

    i32 rowY = 12;
    CreateWindowW(L"STATIC", L"Background:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90,
                  22, hwndSettings_, nullptr, hInst, nullptr);
    btnBgColor_ = CreateWindowW(L"BUTTON", L"", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, 108, rowY + 1,
                                28, 20, hwndSettings_, (HMENU)(INT_PTR)IDC_BGCOLOR, hInst, nullptr);

    rowY += 38;
    CreateWindowW(L"STATIC", L"Exposure:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90, 22,
                  hwndSettings_, nullptr, hInst, nullptr);
    sldExposure_ = CreateWindowExW(0, TRACKBAR_CLASSW, L"",
                                   WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_NOTICKS, 108, rowY, 150,
                                   24, hwndSettings_, (HMENU)(INT_PTR)IDC_EXPOSURE, hInst, nullptr);
                 ...
                 (LPARAM)(i32)(service_.Settings().GetTonemapExposure() * 100.0f));
    {
        ...
        lblExposure_ = CreateWindowW(L"STATIC", buf, WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 264,
                                     rowY, 44, 22, hwndSettings_, nullptr, hInst, nullptr);
    }

    rowY += 38;
    CreateWindowW(L"STATIC", L"SND Volume:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90,
                  22, hwndSettings_, nullptr, hInst, nullptr);
    sldSndVolume_ = CreateWindowExW(
        0, TRACKBAR_CLASSW, L"", WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_NOTICKS, 108, rowY, 150, 24,
        hwndSettings_, (HMENU)(INT_PTR)IDC_SND_VOLUME, hInst, nullptr);
                 ...
                 (LPARAM)(i32)(service_.Sound().GetVolume() * 100.0f));
    {
        ...
        lblSndVolume_ = CreateWindowW(L"STATIC", buf, WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 264,
                                      rowY, 44, 22, hwndSettings_, nullptr, hInst, nullptr);
    }

    rowY += 38;
    chkLoopNonLoop_ = CreateWindowW(
        L"BUTTON", L"Loop NonLooping animations", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 12, rowY,
        280, 22, hwndSettings_, (HMENU)(INT_PTR)IDC_LOOP_NONLOOP, hInst, nullptr);
    SendMessageW(chkLoopNonLoop_, BM_SETCHECK, loopNonLoopingPolicy_ ? BST_CHECKED : BST_UNCHECKED,
                 0);

    rowY += 38;
    CreateWindowW(L"STATIC", L"Time of Day:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90,
                  22, hwndSettings_, nullptr, hInst, nullptr);
    sldTimeOfDay_ = CreateWindowExW(
        0, TRACKBAR_CLASSW, L"", WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_NOTICKS, 108, rowY, 150, 24,
        hwndSettings_, (HMENU)(INT_PTR)IDC_TIME_OF_DAY, hInst, nullptr);
    {
        const f32 hpd =
            service_.GetDncService() ? service_.GetDncService()->GetHoursPerDay() : 24.0f;
        const i32 rangeHi = static_cast<i32>(hpd * 100.0f);
        SendMessageW(sldTimeOfDay_, TBM_SETRANGE, TRUE, MAKELPARAM(0, rangeHi));
        const f32 tod = service_.GetDncService() ? service_.GetDncService()->GetTimeOfDay() : 12.0f;
        SendMessageW(sldTimeOfDay_, TBM_SETPOS, TRUE, (LPARAM)(i32)(tod * 100.0f));
        wchar_t buf[16];
        const i32 hh = static_cast<i32>(tod) % 24;
        const i32 mm = static_cast<i32>((tod - std::floor(tod)) * 60.0f) % 60;
        swprintf_s(buf, L"%02d:%02d", hh, mm);
        lblTimeOfDay_ = CreateWindowW(L"STATIC", buf, WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 264,
                                      rowY, 44, 22, hwndSettings_, nullptr, hInst, nullptr);
    }

    rowY += 38;
    chkAnimateTod_ =
        CreateWindowW(L"BUTTON", L"Animate TOD", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 12, rowY,
                      280, 22, hwndSettings_, (HMENU)(INT_PTR)IDC_ANIMATE_TOD, hInst, nullptr);
    {
        const bool animating =
            service_.GetDncService() && service_.GetDncService()->GetTodScale() > 0.0f;
        SendMessageW(chkAnimateTod_, BM_SETCHECK, animating ? BST_CHECKED : BST_UNCHECKED, 0);
    }

    rowY += 38;
    CreateWindowW(L"STATIC", L"IBL:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90, 22,
                  hwndSettings_, nullptr, hInst, nullptr);
    cmbIblMode_ =
        CreateWindowW(L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, 108,
                      rowY, 200, 200, hwndSettings_, (HMENU)(INT_PTR)IDC_IBL_MODE, hInst, nullptr);
    ...
    SendMessageW(cmbIblMode_, CB_SETCURSEL, static_cast<WPARAM>(service_.Settings().GetIblMode()),
                 0);

    rowY += 38;
    CreateWindowW(L"STATIC", L"Shadows:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90, 22,
                  hwndSettings_, nullptr, hInst, nullptr);
    cmbShadows_ =
        CreateWindowW(L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, 108,
                      rowY, 200, 200, hwndSettings_, (HMENU)(INT_PTR)IDC_SHADOWS, hInst, nullptr);
    ...
    rowY += 38;
    CreateWindowW(L"STATIC", L"DNC Model:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90,
                  22, hwndSettings_, nullptr, hInst, nullptr);
    editDncPath_ = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"",
                                   WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, 108, rowY + 1, 200, 20,
                                   hwndSettings_, (HMENU)(INT_PTR)IDC_DNC_PATH, hInst, nullptr);
    btnDncReset_ =
        CreateWindowW(L"BUTTON", L"Reset", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 314, rowY, 54, 22,
                      hwndSettings_, (HMENU)(INT_PTR)IDC_DNC_RESET, hInst, nullptr);
    ...
    rowY += 38;
    CreateWindowW(L"STATIC", L"Backend:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90, 22,
                  hwndSettings_, nullptr, hInst, nullptr);
    cmbDefaultBackend_ = CreateWindowW(
        L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, 108, rowY, 200,
        200, hwndSettings_, (HMENU)(INT_PTR)IDC_DEFAULT_BACKEND, hInst, nullptr);
    ...
    rowY += 38;
    CreateWindowW(L"STATIC", L"Device:", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 12, rowY, 90, 22,
                  hwndSettings_, nullptr, hInst, nullptr);
    cmbPreferredDev_ = CreateWindowW(
        L"COMBOBOX", L"", WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL, 108, rowY, 260,
        200, hwndSettings_, (HMENU)(INT_PTR)IDC_PREFERRED_DEVICE, hInst, nullptr);
    ...
    SendMessageW(cmbPreferredDev_, CB_ADDSTRING, 0, (LPARAM)L"(Auto — highest VRAM)");
    {
        const auto names = gfx::EnumerateDevices(service_.Settings().DefaultBackend());
        for (const auto& n : names) {
            const i32 wlen = ::MultiByteToWideChar(CP_UTF8, 0, n.c_str(), -1, nullptr, 0);
            if (wlen <= 0)
                continue;
            std::wstring w(wlen, L'\0');
            ::MultiByteToWideChar(CP_UTF8, 0, n.c_str(), -1, w.data(), wlen);
    ...
    }
    {
        ...
        if (!cur.empty()) {
            const i32 wlen = ::MultiByteToWideChar(CP_UTF8, 0, cur.c_str(), -1, nullptr, 0);
            if (wlen > 0) {
                std::wstring w(wlen, L'\0');
                ::MultiByteToWideChar(CP_UTF8, 0, cur.c_str(), -1, w.data(), wlen);
                const LRESULT found = SendMessageW(cmbPreferredDev_, CB_FINDSTRINGEXACT, (WPARAM)-1,
                                                   (LPARAM)w.c_str());
                if (found != CB_ERR)
                    sel = static_cast<i32>(found);
            }
        }
    ...
    rowY += 38;
    chkGraphicsDebug_ =
        CreateWindowW(L"BUTTON", L"Graphics Debug (validation, restart required)",
                      WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 12, rowY, 320, 22, hwndSettings_,
...
LRESULT RenderWindow::HandleSettingsMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        if (id == IDC_LOOP_NONLOOP) {
            const bool on =
                chkLoopNonLoop_ && SendMessageW(chkLoopNonLoop_, BM_GETCHECK, 0, 0) == BST_CHECKED;
        ...
        if (id == IDC_ANIMATE_TOD) {
            const bool on =
                chkAnimateTod_ && SendMessageW(chkAnimateTod_, BM_GETCHECK, 0, 0) == BST_CHECKED;
            if (auto* dnc = service_.GetDncService())
                dnc->SetTodScale(on ? 1.0f : 0.0f);
            SaveSettingsIni(service_, loopNonLoopingPolicy_);
            return 0;
        }
        ...
        if (id == IDC_PREFERRED_DEVICE && HIWORD(wParam) == CBN_SELCHANGE && cmbPreferredDev_) {
            ...
            if (sel <= 0) {
                ...
                if (wlen > 0) {
                    std::wstring w(wlen, L'\0');
                    SendMessageW(cmbPreferredDev_, CB_GETLBTEXT, sel, (LPARAM)w.data());
                    const i32 u8len = ::WideCharToMultiByte(CP_UTF8, 0, w.data(), wlen, nullptr, 0,
                                                            nullptr, nullptr);
                    if (u8len > 0) {
                        std::string utf8(u8len, '\0');
                        ::WideCharToMultiByte(CP_UTF8, 0, w.data(), wlen, utf8.data(), u8len,
                                              nullptr, nullptr);
        ...
        if (id == IDC_DNC_PATH && HIWORD(wParam) == EN_KILLFOCUS && editDncPath_) {
            ...
            if (n >= 0) {
                const i32 u8len =
                    ::WideCharToMultiByte(CP_UTF8, 0, buf, n, nullptr, 0, nullptr, nullptr);
                std::string utf8(u8len, '\0');
                if (u8len > 0) {
                    ::WideCharToMultiByte(CP_UTF8, 0, buf, n, utf8.data(), u8len, nullptr, nullptr);
                }
                if (auto* dnc = service_.GetDncService()) {
                    dnc->SetUnitMdl(utf8);
                    SaveSettingsIni(service_, loopNonLoopingPolicy_);
