    ...
    for (std::size_t m = 0; m < source->materials.size(); ++m) {
        ...
        for (u32 y = 0; y < height; ++y) {
            ...
            for (u32 x = 0; x < width; ++x) {
                ...
                // The forward bake whitened the albedo under the team share
                // (`lerp(white, albedo, 1-team)`); divide it back out where
                // enough of it survives to recover. Under a near-full share
                // the albedo is unknowable and stays what the map holds.
                const f32 keep = std::max(1.0f - share, 0.10f);
                f32 albedo[3];
                for (int c = 0; c < 3; ++c) {
                    const f32 painted = basePx[at + static_cast<std::size_t>(c)] / 255.0f;
                    albedo[c] = std::clamp(1.0f + (painted - 1.0f) / keep, 0.0f, 1.0f);
                }

                for (int c = 0; c < 3; ++c) {
                    teamPx[at + static_cast<std::size_t>(c)] =
                        static_cast<u8>(albedo[c] * 255.0f + 0.5f);
                ...
                // Reforged masks team HIGH; StarCraft II shows team where the
                // diffuse alpha is LOW (`lerp(teamColor, diffuse, texel.a)` in
                // the standard shader) — so the share INVERTS on the way over.
                teamPx[at + 3] = static_cast<u8>(std::clamp(1.0f - share, 0.0f, 1.0f) * 255.0f +
                                                 0.5f);
                specPx[at + 3] = 255;
            }
        }
        ...
        // (`psmaterial.fx` DecodeNormal reads .ag) -- the war3 mod's own maps
        // measure A=x, G=y, B=0. The restatement is a channel MOVE and nothing
        // else: the axes agree between the engines. It has to be a move
        ...
        derived.MutableCommon().body = std::move(body);
...
Wc3ToSc2Result RestateWc3AsSc2(wem::Document& document, io::IContentProvider* provider,
    ...
    // The team glow's texture. Replaceable 2 names a file the GAME supplies
    ...
    // replaceable id stays for the material fold to read (the fold is what
    // turns the layer into a TeamColorEmissiveAdd emissive). Replaceable 1
    // stays pathless on purpose: the team COLOUR needs no texture at all.
    for (wem::TextureRef& ref : document.textures) {
        if (ref.replaceableId == 2 && ref.path.empty()) {
            ref.path = "ReplaceableTextures/TeamGlow/TeamGlow00.blp";
