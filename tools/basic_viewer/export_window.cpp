...
// Formatted into a rotating scratch buffer — ImGui takes the pointer and
// draws it inside the same call, so one buffer per nesting level is plenty.
const char* Fmt(const char* fmt, ...) {
    ...
    va_list args;
    va_start(args, fmt);
    std::vsnprintf(buf, sizeof(bufs[0]), fmt, args);
    va_end(args);
    return buf;
}
...
constexpr ImU32 kAmber = IM_COL32(230, 170, 60, 255);
constexpr ImU32 kRed = IM_COL32(230, 90, 80, 255);
...
void ExportWindow::LoadIfNeeded() {
    ...
    LoadExportRecipe(recipe_, app_.SequenceNames(), &report);
    resolvedAgainst_ = app_.SequenceNames();
...
void ExportWindow::Open(i32 seedSequence) {
    ...
    if (folderBuf_.empty() && !app_.CurrentModelPath().empty())
        folderBuf_ = io::PathToUtf8(app_.CurrentModelPath().parent_path());
    open_ = true;
}
...
void ExportWindow::Save() {
    ...
    SaveExportRecipe(recipe_, app_.SequenceNames(), app_.CurrentModelPath());
}

void ExportWindow::OnModelChanged() {
    const auto& names = app_.SequenceNames();
...
void ExportWindow::RebuildSchedule() {
    schedule_ = ExportSchedule::Build(recipe_, app_.SequenceRanges());
}
...
void ExportWindow::ApplyPreset(i32 preset) {
    preset_ = preset;
    const auto& names = app_.SequenceNames();
    const renderer::model::Actor* focus = app_.FocusActorPtr();
...
void ExportWindow::BuildPresetRow() {
    ...
    if (ImGui::Button(T("dialog.export.recipe_save"))) {
        ...
        if (NFD::SaveDialog(out, filter, 1, nullptr, "recipe.ini") == NFD_OKAY) {
            recipe_.output.folder = io::FsPathFromUtf8(folderBuf_);
            recipe_.output.nameTemplate = nameBuf_;
            WriteExportRecipeFile(io::FsPathFromUtf8(out.get()), recipe_, app_.SequenceNames(),
                                  app_.CurrentModelPath());
        }
    }
    ImGui::SameLine();
    if (ImGui::Button(T("dialog.export.recipe_load"))) {
        ...
        if (NFD::OpenDialog(out, filter, 1) == NFD_OKAY) {
            ExportRecipeLoadReport report;
            if (ReadExportRecipeFile(io::FsPathFromUtf8(out.get()), recipe_, app_.SequenceNames(),
                                     &report)) {
                folderBuf_ = io::PathToUtf8(recipe_.output.folder);
                nameBuf_ = recipe_.output.nameTemplate;
                resMode_ = (recipe_.output.width > 0 && recipe_.output.height > 0) ? 1 : 0;
...
void ExportWindow::BuildQueue() {
    ...
    if (ImGui::Button(T("dialog.export.add"))) {
        addSelected_.assign(app_.SequenceNames().size(), 0);
        addSearch_.clear();
        openAddPopup_ = true;
    }
    ImGui::SameLine();
...
void ExportWindow::BuildAddClipPopup() {
    ...
    char buf[128];
    std::snprintf(buf, sizeof(buf), "%s", addSearch_.c_str());
    ImGui::SetNextItemWidth(260);
    if (ImGui::InputTextWithHint("##search", T("dialog.export.search"), buf, sizeof(buf)))
        addSearch_ = buf;

    const std::string needle = [&] {
        std::string s = addSearch_;
        for (char& c : s)
            c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        return s;
    }();

    ImGui::BeginChild("##addlist", ImVec2(300, 260), ImGuiChildFlags_Borders);
    for (i32 i = 0; i < static_cast<i32>(names.size()); ++i) {
        std::string lower = names[static_cast<usize>(i)];
        for (char& c : lower)
            c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
...
void ExportWindow::BuildTimeline() {
    ...
    static const ImU32 kBlockA = IM_COL32(80, 110, 150, 255);
    static const ImU32 kBlockB = IM_COL32(70, 130, 120, 255);
    static const ImU32 kFillCol = IM_COL32(90, 90, 100, 200);
    ...
    for (usize s = 0; s < clips.size(); ++s) {
        ...
        const ImU32 col = isFill ? kFillCol : ((s % 2) ? kBlockB : kBlockA);
    ...
    for (i32 s = 0; s <= seconds; ++s) {
        ...
        dl->AddLine(ImVec2(x, origin.y + height - 5.0f), ImVec2(x, origin.y + height),
    ...
    if (ImGui::IsItemActive() && width > 0.0f && !app_.IsExportRunning()) {
        ...
        app_.ScrubExportRecipe(recipe_, scrubFrame_, true);
        previewing_ = false;
    }
...
void ExportWindow::BuildCamera() {
    ...
    const auto& presets = app_.CameraPresetNamesUtf8();
...
void ExportWindow::BuildOutput() {
    ...
    ImGui::Indent(12.0f);
    {
        ...
        std::string why;
        if (!ExportFormatAvailable(o.format, &why))
            Warn(kRed, why.c_str());
    }
    ...
    if (o.overrideBackground) {
        ImGui::SameLine();
        f32 col[3] = {o.backgroundR / 255.0f, o.backgroundG / 255.0f, o.backgroundB / 255.0f};
        ImGui::SetNextItemWidth(160);
        if (ImGui::ColorEdit3("##bg", col, ImGuiColorEditFlags_NoInputs)) {
            o.backgroundR = static_cast<u8>(col[0] * 255.0f);
            o.backgroundG = static_cast<u8>(col[1] * 255.0f);
            o.backgroundB = static_cast<u8>(col[2] * 255.0f);
            MarkCustom();
        }
    }
    ImGui::EndDisabled();
    ...
    {
        char buf[1024];
        std::snprintf(buf, sizeof(buf), "%s", folderBuf_.c_str());
    ...
    {
        char buf[256];
        std::snprintf(buf, sizeof(buf), "%s", nameBuf_.c_str());
        ImGui::SetNextItemWidth(340);
        if (ImGui::InputTextWithHint("##nametmpl", DefaultNameTemplate(o.format), buf,
                                     sizeof(buf))) {
            nameBuf_ = buf;
            MarkCustom();
        }
...
std::string ExportWindow::ResolvedOutputPath() const {
    NameTokens tokens;
    tokens.model = SanitizeExportName(io::PathToUtf8(app_.CurrentModelPath().stem()));
    i32 resolved = 0;
...
void ExportWindow::BuildFooter() {
    ...
    if (hasReport_) {
        ImGui::Separator();
        Warn(reportOk_ ? IM_COL32(120, 200, 120, 255) : kRed, reportLine_.c_str());
    ...
    const f32 actionsX = ImGui::GetCursorPosX() + ImGui::GetContentRegionAvail().x - 200.0f;
    ...
    const bool canExport =
        ...
        ExportFormatAvailable(recipe_.output.format) && !app_.IsExportRunning();
    ImGui::SameLine(actionsX);
    ImGui::BeginDisabled(!canExport);
    if (ImGui::Button(T("dialog.export.export"), ImVec2(110, 0))) {
        previewing_ = false;
        recipe_.output.folder = io::FsPathFromUtf8(folderBuf_);
        recipe_.output.nameTemplate = nameBuf_;
        Save();
        app_.RequestAnimationExport(recipe_);
    }
    ImGui::EndDisabled();
    ImGui::SameLine();
    if (ImGui::Button(T("app.close"), ImVec2(80, 0)))
        Close();
...
void ExportWindow::Tick(f32 dtSec) {
    if (!previewing_ || !open_ || app_.IsExportRunning())
        return;
    const ExportSchedule schedule = ExportSchedule::Build(recipe_, app_.SequenceRanges());
    ...
    app_.ScrubExportRecipe(recipe_, previewFrame_, true);
...
void ExportWindow::Build() {
    ...
    if (app_.ConsumeExportFinished()) {
        const ExportReport& r = app_.LastExportReport();
        hasReport_ = true;
        reportOk_ = r.ok;
    ...
    ImGui::SetNextWindowSize(ImVec2(720, 780), ImGuiCond_FirstUseEver);
    bool stayOpen = true;
