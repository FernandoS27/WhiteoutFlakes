...
std::string TrimWs(std::string_view s) {
    ...
    usize a = 0, b = s.size();
    while (a < b && isWs(static_cast<unsigned char>(s[a])))
        ++a;
    while (b > a && isWs(static_cast<unsigned char>(s[b - 1])))
        --b;
...
const ExportFormatInfo& GetExportFormatInfo(ExportFormat format) {
    ...
    static constexpr ExportFormatInfo kInfo[kExportFormatCount] = {
        {"PNG frames", "", ExportLayout::Frames, "png"},
        {"Animated GIF", ".gif", ExportLayout::Animated, "gif"},
        {"Animated PNG (APNG)", ".apng", ExportLayout::Animated, "apng"},
        {"Animated WebP", ".webp", ExportLayout::Animated, "webp"},
        {"Sprite sheet (PNG)", ".png", ExportLayout::Sheet, "sheet"},
        {"Video (MP4 / H.264)", ".mp4", ExportLayout::Video, "mp4"},
        {"Video (WebM / VP9)", ".webm", ExportLayout::Video, "webm"},
    };
    const i32 i = static_cast<i32>(format);
    return kInfo[(i >= 0 && i < kExportFormatCount) ? i : 0];
}

bool ParseExportFormat(std::string_view token, ExportFormat& out) {
    const std::string want = LowerAscii(TrimWs(token));
...
bool NameTemplateCollides(std::string_view tmpl, ExportFormat format) {
    if (LayoutOf(format) != ExportLayout::Frames)
        return false;
    return LowerAscii(tmpl).find("{index") == std::string::npos;
}
...
std::string ExpandNameTemplate(std::string_view tmpl, const NameTokens& tokens,
    ...
    for (usize i = 0; i < tmpl.size();) {
        ...
        const std::string name = LowerAscii(body.substr(0, colon));
        i32 width = 0;
...
i32 ResolveSequenceKey(std::span<const std::string> names, std::string_view key) {
    ...
    const std::string want = LowerAscii(TrimWs(name));
    for (i32 i = 0; i < static_cast<i32>(names.size()); ++i)
        if (LowerAscii(TrimWs(names[static_cast<usize>(i)])) == want)
            return i;
    return -1;
}
