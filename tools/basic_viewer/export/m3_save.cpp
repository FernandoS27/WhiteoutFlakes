...
/// A data-driven material no map names yet.
...
std::size_t MergeAnimationChunks(m3::Model& dst, const m3::Model& src, wem::Diagnostics& lossy,
    ...
    if ((srcSeqVersion <= 1) != (seqVersion <= 1)) {
        lossy.warn(wem::DiagCode::LossyKindConversion,
                   label + ": SEQS v" + std::to_string(srcSeqVersion) +
...
void PruneUnreferencedDataDriven(m3::Model& model) {
    ...
    std::vector<u32> remap(model.dataDrivenMaterials.size(), kUnmapped);
    ...
    for (m3::MaterialMap& map : model.materialMaps) {
        if (map.materialType != m3::MaterialType::DataDriven || map.materialIndex >= remap.size())
            continue;
        if (remap[map.materialIndex] == kUnmapped) {
...
void ExportUsedTextures(const m3::Model& model, const M3SaveRequest& request,
    ...
    // CollectM3Textures dedupes by (cube, linear) too; a path is written once.
    std::unordered_set<std::string> written;
    for (const io::M3TextureRef& ref : io::CollectM3Textures(model)) {
        const std::string relative = export_text::ForwardSlashes(ref.path);
        if (!written.insert(export_text::Lower(relative)).second)
            continue;
        ...
        const std::optional<TextureFile> file =
            ReadTextureFile(*request.provider, ContentRef::FromPath(ref.path));
        if (!file) {
            report.diagnostics.warn(wem::DiagCode::TextureUnresolved,
                                    "texture '" + ref.path + "' is not readable");
            ++counters.failed;
            continue;
        }

        fs::path outRel = io::FsPathFromUtf8(relative);
        if (!file->extension.empty())
            outRel.replace_extension(file->extension);
        ...
        if (!WriteFileCreatingDirs(outFile, file->bytes)) {
...
M3SaveReport SaveModelAsM3(const M3SaveRequest& request) {
    ...
    const M3SaveOptions& options = request.options;
    ...
    if (options.mergeAnimations) {
        ...
        for (std::size_t i = 0; i < attached.size(); ++i) {
            ...
            report.mergedSequences +=
                MergeAnimationChunks(model, *anim, report.diagnostics, attached[i].label);
            ++report.mergedFiles;
        }
    }

    if (options.convertToSc2) {
        ...
        for (std::string& reason : conversion.lossy)
            report.diagnostics.warn(wem::DiagCode::LossyKindConversion, std::move(reason));
        model = std::move(conversion.model);
    }

    report.version = model.getVersion();

    report.error = WriteModelFile(request.outPath, [&](const std::string& path) {
        m3::Writer writer;
