...
std::vector<u8> WrapPcmS16AsWav(const drflac_int16* samples, drflac_uint64 totalFrames,
                                u32 channels, u32 sampleRate) {
    const u32 payloadBytes =
        static_cast<u32>(totalFrames) * static_cast<u32>(channels) * sizeof(i16);
...
std::optional<std::vector<u8>> ResolveSoundBytes(const IContentProvider& cp,
    ...
    auto fetch = [&](const std::string& p, bool decodeFlac) -> std::optional<std::vector<u8>> {
        if (p.empty())
            return std::nullopt;
    ...
    auto candidates = [&](const std::string& path) -> std::vector<std::pair<std::string, bool>> {
    ...
    for (usize i = 0; i < n; ++i) {
        for (const auto& [path, decodeFlac] : candidates(entry.filePaths[(start + i) % n])) {
            if (auto bytes = fetch(path, decodeFlac))
                return bytes;
            if (attemptedOut) {
...
WindowsSoundEmitter::WindowsSoundEmitter(const IContentProvider* content) : content_(content) {}
...
void WindowsSoundEmitter::Play(const io::SndEntry& entry, const Vector3f&) {
    if (!content_)
        return;
    ...
    if (!bytes) {
        ...
        std::fprintf(stderr, "[WDEX sound] missing -- %zu source path(s); probed: %s\n",
    ...
    PlaySoundW(reinterpret_cast<LPCWSTR>(currentBuffer_.data()), nullptr,
