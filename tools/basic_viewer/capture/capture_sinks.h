...
public:
    ...
    /// False when the sink cannot start; `Finish` then says why.
    ...
    /// Writes what the frames amount to. Called once, after the last frame.
    virtual ExportSinkResult Finish() = 0;
};
...
/// @brief True when an external `ffmpeg` is on PATH — the video formats grey
///        out without it, the way WebP does when it was not compiled in.
///        Probed once and cached; the probe starts a process and waits for
///        it, so a UI thread asks `FfmpegProbed` instead.
bool FfmpegAvailable();

/// @brief The cached answer of `FfmpegAvailable`; nothing before it has run.
std::optional<bool> FfmpegProbed();

/// @brief Whether this build and machine can write a format.
struct FormatAvailability {
    bool available = true;
    /// A short user-facing sentence when not.
    std::string reason;
};

/// @brief Whether @p format can be written. A video format waits on the ffmpeg
///        probe.
FormatAvailability ExportFormatAvailable(ExportFormat format);

/// @brief `ExportFormatAvailable` for a UI thread: a video format is
///        unavailable, and says it is being checked, until the probe answered.
