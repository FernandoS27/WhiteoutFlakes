...
struct ExportHost {
    ...
    /// @brief Dispatch window events and report whether the user asked to stop.
    ///        The export owns the frame loop, so this is also what keeps the
    ///        window alive while it runs. Empty never cancels.
    std::function<bool()> pollCancel;
};
