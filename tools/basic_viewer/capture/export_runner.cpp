...
public:
    ...
    ~ExportStateGuard() {
        ...
        const tools::Rgb8 bg = tools::UnpackBgr(savedBg_);
        svc_.Settings().SetBackgroundColor(bg.r, bg.g, bg.b);
...
/// Frames of simulation a pre-roll may warm: a minute at 60 fps.
constexpr i32 kMaxPreRollFrames = 3600;
/// Progress is reported every this many percent.
constexpr i32 kProgressStepPercent = 2;

void RenderAndPresent(const ExportHost& host) {
    renderer::RenderService& svc = *host.service;
    Viewport vp;
    vp.scene = host.scene;
    vp.target = host.target;
    vp.camera = &svc.SceneAt(host.scene).Camera();
    svc.Pipeline().RenderViewport(vp);
    svc.Pipeline().Present(host.target);
}

void ApplyOrbitPose(Camera& cam, const OrbitBase& base, f32 yawDeg) {
    cam.SetOrbitalMode();
    cam.SetTarget(base.target);
    cam.SetPitch(base.pitchRad);
    cam.SetDistance(base.distance);
    cam.SetYaw(Deg2Rad(base.yawDeg + yawDeg));
}

// One export, stage by stage. Everything it changes about the viewer is held by
// the state guard, which puts it back when the run is destroyed — after the
// sink has finished and the sidecar is written, whatever path got there.
class ExportRun {
public:
    ExportRun(const ExportRecipe& recipe, const ExportHost& host) : recipe_(recipe), host_(host) {}

    ExportReport Run() {
        const auto t0 = std::chrono::steady_clock::now();
        if (!Validate())
            return report_;
        guard_.emplace(host_, *host_.service, *host_.hero);
        ApplyOutputState();
        SetUpCamera();
        if (!SetUpSink())
            return report_;
        CaptureFrames();
        Finish();
        report_.elapsedSec = std::chrono::duration<f64>(std::chrono::steady_clock::now() - t0).count();
        return report_;
    }

private:
    bool Validate() {
        if (!host_.service || !host_.hero || !host_.hero->animation.HasSource()) {
            report_.error = "no animated model loaded";
            return false;
        }
        seqs_ = host_.hero->animation.Sequences();
        if (seqs_.empty()) {
            report_.error = "the model has no animation sequences";
            return false;
        }
        schedule_ = ExportSchedule::Build(recipe_, seqs_);
        for (const std::string& w : schedule_.Warnings())
            std::fprintf(stderr, "[viewer] Export: %s\n", w.c_str());

        totalFrames_ = schedule_.TotalFrameCount();
        if (totalFrames_ <= 0) {
            report_.error = "the recipe schedules no frames";
            return false;
        }
        if (totalFrames_ > kMaxExportFrames) {
            report_.error = "the recipe schedules " + std::to_string(totalFrames_) + " frames, past the " +
                            std::to_string(kMaxExportFrames) + "-frame cap";
            return false;
        }
        report_.framesRequested = schedule_.CapturedFrameCount();

        const FormatAvailability availability = ExportFormatAvailable(recipe_.output.format);
        if (!availability.available) {
            report_.error = availability.reason;
            return false;
        }
        std::error_code ec;
        fs::create_directories(recipe_.output.folder, ec);
        report_.folder = recipe_.output.folder;
        return true;
    }

    void ApplyOutputState() {
        renderer::RenderService& svc = *host_.service;
        const ExportOutput& out = recipe_.output;
        const bool customRes = out.width > 0 && out.height > 0 &&
                               (out.width != guard_->SavedWidth() || out.height != guard_->SavedHeight());
        if (customRes) {
            svc.Pipeline().ResizePrimaryTarget(out.width, out.height);
            guard_->NoteResize();
        }
        width_ = svc.Pipeline().Width();
        height_ = svc.Pipeline().Height();
        ...
        if (out.overrideBackground && !out.transparent)
            svc.Settings().SetBackgroundColor(out.backgroundR, out.backgroundG, out.backgroundB);

        // The export drives the clock itself, so the transport must not be able
        // to freeze or stretch it — the reason playbackSpeed is driven per frame
        // rather than left where the user had it.
        renderer::SceneManager& clock = svc.SceneAt(host_.scene);
        clock.SetPlaybackState(PlaybackState::Playing);
        clock.SetTimeScale(1.0f);
    }

    void SetUpCamera() {
        const ExportCameraMotion& camera = recipe_.camera;
        if (camera.mode == ExportCameraMode::Preset && camera.preset >= 0 && host_.activateCameraPreset) {
            host_.activateCameraPreset(camera.preset);
            guard_->NoteCameraMoved();
        } else if (camera.mode == ExportCameraMode::Orbit) {
            orbitBase_ = ResolveOrbitBase(recipe_, *host_.service, host_.scene, host_.hero);
            guard_->NoteCameraMoved();
            if (orbitBase_.clampedDistance)
                std::fprintf(stderr, "[viewer] Export: orbit distance clamped to %.0f (camera limit)\n",
                             orbitBase_.distance);
            if (orbitBase_.clampedPitch)
                std::fprintf(stderr, "[viewer] Export: orbit pitch clamped to %.1f deg\n",
                             orbitBase_.pitchRad * 180.0f / kPi);
        }
        orbitCamera_ = camera.mode == ExportCameraMode::Orbit && camera.subject == OrbitSubject::Camera;
        orbitModel_ = camera.mode == ExportCameraMode::Orbit && camera.subject == OrbitSubject::Model;
    }

    bool SetUpSink() {
        naming_ = BuildNaming(recipe_, host_, schedule_, width_, height_);
        sink_ = MakeExportSink(recipe_.output);
        setup_.format = recipe_.output.format;
        setup_.folder = recipe_.output.folder;
        setup_.fps = schedule_.OutputFps();
        setup_.transparent = recipe_.output.transparent;
        setup_.sheetColumns = recipe_.output.sheetColumns;
        setup_.expectedFrames = schedule_.CapturedFrameCount();
        setup_.framesPerPass =
            (schedule_.AngleCount() > 1) ? schedule_.CapturedFrameCount() / schedule_.AngleCount() : 0;
        setup_.log = [](const std::string& m) { std::fprintf(stderr, "[viewer] %s\n", m.c_str()); };

        std::vector<std::string> unknown;
        setup_.baseName = ExpandNameTemplate(naming_.tmpl, naming_.tokens, &unknown);
        setup_.frameName = [this](const ExportStep& step) {
            NameTokens t = naming_.tokens;
            t.index = step.frameIndex;
            t.clipFrame = step.clipFrameIndex;
            t.angle = step.angle;
            if (step.clipIndex >= 0 && step.clipIndex < static_cast<i32>(naming_.clipNames.size()))
                t.clip = naming_.clipNames[static_cast<usize>(step.clipIndex)];
            return ExpandNameTemplate(naming_.tmpl, t);
        };
        for (const std::string& u : unknown)
            std::fprintf(stderr, "[viewer] Export: unknown name token '{%s}' left literal\n", u.c_str());
        if (NameTemplateCollides(naming_.tmpl, recipe_.output.format))
            std::fprintf(stderr, "[viewer] Export: '%s' has no {index}, so frames will overwrite each other\n",
                         naming_.tmpl.c_str());

        if (sink_->Begin(setup_))
            return true;
        const ExportSinkResult r = sink_->Finish();
        report_.error = r.ok() ? "the output encoder could not start" : r.error;
        return false;
    }

    /// One step of the world: the clip switch, the scene clock, the pose and the
    /// camera, then the UI frame RenderFrame composites.
    void StepWorld(ClipDriver& driver, const ExportStep& step) {
        renderer::RenderService& svc = *host_.service;
        driver.BeginFrame(step);
        if (orbitModel_) {
            // Rotating the world transform is *motion* as far as cloth and
            // ragdolls are concerned, so a cloaked model shows the cape reacting
            // to the spin. The camera is the default subject for that reason.
            host_.hero->worldTransform = Matrix44f::rotation_z(Deg2Rad(step.yawDeg));
        }
        const f32 dt = static_cast<f32>(step.dtMs) / 1000.0f;
        svc.SceneAt(host_.scene).Update(dt);
        driver.AfterUpdate(step);
        driver.EndFrame();
        svc.Ticker().Tick(svc.SceneAt(host_.scene), dt);
        svc.SetActiveScene(host_.scene); // Tick restored the default scene

        if (orbitCamera_) {
            ApplyOrbitPose(svc.SceneAt(host_.scene).Camera(), orbitBase_, step.yawDeg);
        } else if (host_.applyCameraPreset) {
            // Not just in Preset mode: "keep the current view" has to keep
            // *tracking* a preset the user activated, or the camera freezes
            // wherever the animator last put it. A no-op for the free camera.
            host_.applyCameraPreset();
        }

        if (recipe_.output.captureUi && host_.buildUiFrame)
            host_.buildUiFrame();
        else if (host_.buildEmptyFrame)
            host_.buildEmptyFrame();
    }

    /// Warm the simulation without recording it.
    void PreRoll(ClipDriver& driver) {
        if (recipe_.timing.preRollMs <= 0)
            return;
        const i32 rollFrames = std::clamp(
            static_cast<i32>(std::llround(static_cast<f64>(recipe_.timing.preRollMs) * schedule_.Fps() / 1000.0)), 0,
            kMaxPreRollFrames);
        const ExportStep warm = schedule_.At(0);
        for (i32 i = 0; i < rollFrames; ++i) {
            ExportStep s = warm;
            s.passStart = (i == 0);
            s.clipStart = (i == 0);
            s.dtMs = (i == 0) ? 0
                              : (ExportSchedule::ClockAtFrame(i, schedule_.Fps()) -
                                 ExportSchedule::ClockAtFrame(i - 1, schedule_.Fps()));
            StepWorld(driver, s);
            // Rendered but not captured: the world has to actually step, and the
            // particle stages run off the render tick.
            RenderAndPresent(host_);
        }
    }

    void CaptureFrames() {
        auto& pipeline = host_.service->Pipeline();
        pipeline.EnableFrameCapture(true);
        if (auto* cp = host_.service->SceneAt(host_.scene).ActiveContentProvider())
            cp->Pump();

        ClipDriver driver(*host_.hero, seqs_);
        PreRoll(driver);

        // A UI frame cannot be pipelined — the ImGui renderer uploads through one
        // shared vertex buffer, drained before the next overwrites it. The
        // transparent path renders each pose twice and keys the pair, so it
        // cannot pipeline either.
        const bool pipelineFrames = !recipe_.output.captureUi && !recipe_.output.transparent;
        ringSize_ = pipelineFrames ? pipeline.FrameCaptureRingSize() : 1;
        pending_.reserve(static_cast<usize>(std::max(1, ringSize_)));

        i32 lastReported = -1;
        for (i32 i = 0; i < totalFrames_; ++i) {
            if (host_.pollCancel && host_.pollCancel()) {
                report_.cancelled = true;
                break;
            }
            const ExportStep step = schedule_.At(i);
            StepWorld(driver, step);
            if (!step.capture) {
                // A frame the step skips is still simulated, or the world would
                // advance at a different rate than the schedule computed: render
                // it (the particle stages run off the render tick) and discard it.
                RenderAndPresent(host_);
            } else if (recipe_.output.transparent) {
                CaptureKeyed(step);
            } else {
                CaptureOpaque(step);
            }

            // Progress goes where it is free. An in-viewport overlay would land
            // in the captured image and cost a second pass per frame.
            const i32 pct = (i * 100) / std::max(1, totalFrames_);
            if (pct != lastReported && (pct % kProgressStepPercent) == 0) {
                lastReported = pct;
                if (host_.onProgress)
                    host_.onProgress(i, totalFrames_);
                std::fprintf(stderr, "[viewer] Export: %d/%d (%d%%)\r", i, totalFrames_, pct);
            }
        }
        Drain();
    ...
    void CaptureOpaque(const ExportStep& step) {
        RenderAndPresent(host_);
        const i32 slot = host_.service->Pipeline().LastCapturedSlot();
        if (slot >= 0)
            pending_.push_back({step, slot});
        if (static_cast<i32>(pending_.size()) >= ringSize_)
            Drain();
    }

    /// Each frame rendered twice at the same pose — black backdrop, then white —
    /// and keyed into straight alpha.
    void CaptureKeyed(const ExportStep& step) {
        std::vector<u8> black, white;
        i32 bw = 0, bh = 0, ww = 0, wh = 0;
        if (RenderOver(0, 0, 0, black, bw, bh) && RenderOver(255, 255, 255, white, ww, wh) && bw == ww &&
            bh == wh) {
            sink_->Accept(step, KeyOutBackground(black, white, bw, bh));
            ++report_.framesCaptured;
        }
    }

    bool RenderOver(u8 r, u8 g, u8 b, std::vector<u8>& out, i32& w, i32& h) {
        host_.service->Settings().SetBackgroundColor(r, g, b);
        RenderAndPresent(host_);
        auto& pipeline = host_.service->Pipeline();
    ...
    void Drain() {
        ...
        for (const Pending& p : pending_) {
            ...
            if (!pipeline.DownloadCaptureSlot(p.ringSlot, rgba, w, h) || w <= 0 || h <= 0)
                continue;
            sink_->Accept(p.step, TextureFromRgba(rgba, w, h));
            ++report_.framesCaptured;
        }
        pending_.clear();
    }
    ...
    void Finish() {
        const ExportSinkResult sinkResult = sink_->Finish();
        report_.files = sinkResult.files;
        report_.error = sinkResult.error;
        report_.ok = sinkResult.ok() && sinkResult.framesWritten > 0;
        report_.framesCaptured = sinkResult.framesWritten;

        if (report_.ok && recipe_.output.writeSidecar && !sinkResult.files.empty()) {
            fs::path side = sinkResult.files.front();
            side.replace_extension(".json");
            if (LayoutOf(recipe_.output.format) == ExportLayout::Frames)
                side = recipe_.output.folder / io::FsPathFromUtf8(setup_.baseName + ".json");
            WriteSidecar(side, recipe_, schedule_, host_, sinkResult, width_, height_);
        }
        for (const fs::path& f : report_.files) {
            std::error_code sizeEc;
            const auto sz = fs::file_size(f, sizeEc);
            if (!sizeEc)
                report_.outputBytes += sz;
        }
    }
    ...
    const ExportRecipe& recipe_;
    const ExportHost& host_;
    // First: destroyed last, so the viewer is restored once everything else the
    // run built — the sink included — is gone.
    std::optional<ExportStateGuard> guard_;
    ...
    i32 width_ = 0;
    i32 height_ = 0;
    OrbitBase orbitBase_;
    bool orbitCamera_ = false;
    bool orbitModel_ = false;
    Naming naming_;
...
bool PreviewExportFrame(const ExportRecipe& recipe, const ExportHost& host, i32 frameIndex,
    ...
    if (applyCamera && recipe.camera.mode == ExportCameraMode::Orbit &&
        ...
        ApplyOrbitPose(svc.SceneAt(host.scene).Camera(), base, step.yawDeg);
    }
    return true;
}
