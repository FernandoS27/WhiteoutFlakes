...
LoadedEnvProbe LoadEnvProbeFromFile(gfx::IGFXDevice& gfx, const std::string& absPath,
    ...
    DbgLogf("[WDEX IBL] LoadEnvProbeFromFile OK %s (%zd bytes)\n", absPath.c_str(),
            static_cast<isize>(size));
    return LoadEnvProbeFromBytes(gfx, std::span<const u8>(buf.data(), buf.size()), absPath.c_str(),
...
LoadedEnvProbe LoadEnvProbeFromBytes(gfx::IGFXDevice& gfx, std::span<const u8> bytes,
    ...
    DbgLogf(
        "[WDEX IBL] parsed: type=%s fmt=%s %ux%u arraySize=%u layerCount=%u mipCount=%u srgb=%d\n",
        TypeName(tex.type()), FormatName(tex.format()), tex.width(), tex.height(), tex.arraySize(),
        tex.layerCount(), tex.mipCount(), tex.isSrgb() ? 1 : 0);
...
gfx::TextureHandle CreateStudioEnvProbe(gfx::IGFXDevice& gfx) {
    ...
    const FaceRGB diffColors[6] = {
        {kDiffR, kDiffG, kDiffB}, {kDiffR, kDiffG, kDiffB}, {kDiffR, kDiffG, kDiffB},
        {kDiffR, kDiffG, kDiffB}, {kDiffR, kDiffG, kDiffB}, {kDiffR, kDiffG, kDiffB},
    };
    const FaceRGB specColors[6] = {
        {kSideR, kSideG, kSideB}, {kSideR, kSideG, kSideB}, {kTopR, kTopG, kTopB},
        {kBotR, kBotG, kBotB},    {kSideR, kSideG, kSideB}, {kSideR, kSideG, kSideB},
    };
...
bool WriteDebugFacesDds(const std::string& outPath) {
    ...
    constexpr u8 kEdgeColors[12][4] = {
        {0xFF, 0xA0, 0x40, 0xFF}, {0xA0, 0x40, 0xFF, 0xFF}, {0x40, 0xC0, 0xFF, 0xFF},
        {0x80, 0x60, 0x40, 0xFF}, {0xFF, 0x80, 0xC0, 0xFF}, {0x80, 0xFF, 0x40, 0xFF},
        {0xC0, 0x80, 0x40, 0xFF}, {0x40, 0x60, 0x80, 0xFF}, {0x80, 0xFF, 0xC0, 0xFF},
        {0xC0, 0x80, 0xFF, 0xFF}, {0x80, 0x00, 0x20, 0xFF}, {0x00, 0x60, 0x20, 0xFF},
    };

    constexpr i32 kFaceEdges[6][4] = {
        {E_XY_pp, E_XY_pn, E_XZ_pp, E_XZ_pn}, {E_XY_np, E_XY_nn, E_XZ_nn, E_XZ_np},
        {E_YZ_pn, E_YZ_pp, E_XY_np, E_XY_pp}, {E_YZ_np, E_YZ_nn, E_XY_nn, E_XY_pn},
        {E_YZ_pp, E_YZ_np, E_XZ_np, E_XZ_pp}, {E_YZ_pn, E_YZ_nn, E_XZ_pn, E_XZ_nn},
    };

    Texture tex = Texture::createCubeArray(PixelFormat::RGBA8, kSize, kNumCubes, kMips);
            ...
            for (u32 y = 0; y < h; ++y) {
                for (u32 x = 0; x < w; ++x) {
                    ...
                    if (distMin < border) {
                        ...
                        if (distMin == distTop)
                            c = edgeTop;
                        else if (distMin == distBot)
                            c = edgeBot;
                        else if (distMin == distLf)
                            c = edgeLf;
                        else
                            c = edgeRt;
                    }
