...
// ============================================================================
// Lifetime curves — how a particle's colour, alpha, size and rotation evolve
// between birth (t = 0) and death (t = 1).
//
// Replaces WC3's fixed start/mid/end triple: a curve holds any number of keys
// in normalised time and picks its own interpolation mode. Normalised time is
// what makes a curve independent of the lifespan it is evaluated against, and
// it is the domain M2 and M3 author in.
//
// Two WC3 quirks are preserved as data rather than baked in:
//   * `bias` reproduces its `t = raw*0.99 + 0.005` in-segment sampling skew.
//     Other formats leave it at zero.
//   * cell animation is a separate track type, not a value curve — it wraps and
//     repeats over an integer range instead of interpolating.
// ============================================================================
