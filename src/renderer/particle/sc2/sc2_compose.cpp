...
/// the same packing `SampleColor` produces and OP6 pinned. The vertex
...
std::array<f32, 4> UnpackColor(u32 v) {
    constexpr f32 k = renderer::sc2::kInv255;
...
void ParticleStore::Init(u32 maxParticles) {
    const usize n = maxParticles;
    elements.assign(n, SpawnedElement{});
    vertices.assign(n, GpuVertex{});
    list.next.assign(n, ElementList::kNull);
    list.prev.assign(n, ElementList::kNull);
    // Everything free, in index order. `ElementList::Reset` does the
    ...
    // starts in.
    for (usize i = 0; i < n; ++i)
        list.next[i] = (i + 1 < n) ? static_cast<i32>(i + 1) : ElementList::kNull;
    list.head = ElementList::kSentinel;
    list.tail = ElementList::kNull;
    list.freeHead = n != 0 ? 0 : ElementList::kNull;
    list.freeTail = n != 0 ? static_cast<i32>(n - 1) : ElementList::kNull;
    list.poolCount = 0;
    recycle = RecycleArray{};
}

i32 ParticleStore::Acquire() {
    ...
    if (list.freeHead < 0) {
        list.freeHead = ElementList::kNull;
        list.freeTail = ElementList::kNull;
    }

    // Appended, not prepended: the head stays the OLDEST particle, which is
    // what makes `RetireExpired`'s forward walk stop early and what makes
    // the draw order oldest-first (OP8b).
    list.next[n] = ElementList::kSentinel;
...
QuadCamera CameraFromView(const Matrix44f& v) {
    QuadCamera cam;
...
InitWords InitRuntimeWords(const EmitterDesc& d) {
    ...
    if ((flags & static_cast<u32>(ParticleFlag::ScaleTimeByParent)) != 0)
        w.stateFlags |= renderer::sc2::kStateScaleTimeByParent;
    if ((flags & static_cast<u32>(ParticleFlag::UseLocalTime)) != 0)
        w.stateFlags |= renderer::sc2::kStateUseLocalTime;
    ...
    if (d.motion.forcesFallback != 0)
        w.stateFlags = renderer::sc2::kStateForces;
    if (d.Has(ParticleAdditionalFlag::WorldSpace))
        w.stateFlags |= renderer::sc2::kStateWorldSpace;
    ...
    if (d.motion.forces >= kFirstWorldForceChannel)
        w.stateFlags |= renderer::sc2::kStateWorldForces;
    ...
    // its noise off.
    if (d.motion.noiseAmplitude > renderer::sc2::kNoiseThreshold)
        w.emitFlags |= renderer::sc2::kEmitNoise;
    if ((flags & static_cast<u32>(ParticleFlag::InheritParentVelocity)) != 0)
        w.stateFlags |= renderer::sc2::kStateInheritVelocity;
    // The load already asked `CanUseGpuMotion`; asking again here would be
    // a second spelling of the one selector.
    if (d.motion.analytic)
        w.stateFlags |= renderer::sc2::kStateGpuMotion;
    return w;
}
...
usize BuildQuads(const ParticleStore& store, const QuadBatch& batch,
                    ...
                    bool sortReverse, std::vector<Vertex>& out, SortKey sort,
                    f32 hostScale) {
    ...
    if (sort != SortKey::None) {
        ...
        for (const i32 node : order) {
            const GpuVertex& v = store.vertices[static_cast<usize>(node)];
            i32 key = 0;
            if (sort == SortKey::Height) {
    ...
    for (const i32 node : order) {
        ...
        if (n >= store.vertices.size())
            break;
        const QuadResult q =
            ExpandQuad(QuadInputFrom(store.vertices[n]), batch, camera, flags);
        if (!q.supported)
            continue;
        const Vector4f color{q.color[0], q.color[1], q.color[2], q.color[3]};
        // c0 c1 c2 / c3 c2 c1 over `kCorners` — the same winding every other
...
Crossing CrossSquirtKeys(const EmitterDesc& d, std::span<const ClockSample> players,
    ...
    if (memory.sinks.size() != slots)
        memory.sinks.assign(slots, KeySink{});
    ...
    std::vector<Run> runs;
    std::vector<KeyPlayer> live;
    for (usize s = 0; s < slots; ++s) {
        times.clear();
        values.clear();
        runs.clear();
        for (const ClockSample& p : players) {
        ...
        KeySink& sink = memory.sinks[s];
        if (changed) {
            sink.prime = true;
            CollectCrossedKeys(live, -frameDtMs, sink);
        }
        if (CollectCrossedKeys(live, 0, sink))
            out.bursts[s] = static_cast<u32>(SquirtBurst(sink));
    }
