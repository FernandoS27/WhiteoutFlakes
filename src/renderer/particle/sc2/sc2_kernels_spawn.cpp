...
using renderer::sc2::kDegToRad;
using renderer::sc2::kPi;
using renderer::sc2::kTwoPi;
/// A spline tangent above this |z| takes the world basis.
using renderer::sc2::kVerticalCos;
...
f32 SymRand(renderer::sc2::Rng& rng, f32 h) {
...
f32 ShapeRadius(renderer::sc2::Rng& rng, const SpawnPosInputs& in) {
    if (in.cutout)
...
/// generator, so their stream order shows exactly there: SPEED first (OP5).
/// See SC2_PARTICLE_RE.md §17.8.
f32 DrawOverlay(renderer::sc2::Rng& rng, const Overlay& o, const Variation& v) {
    if (o.type == 0)
        return 0.0f;
    return renderer::sc2::SampleWave(o.type, o.frequency * v.time + v.phase, o.amplitude, &rng);
}

} // namespace

Vector3f SampleSpawnPosition(renderer::sc2::Rng& rng, const SpawnPosInputs& in, Vector3f* normal) {
    switch (in.shape) {
    ...
    case SpawnShape::Sphere: {
        ...
        // `sqrt(1-w²)` and `r·w`, the measurable form of the disassembly's
        // `asinf(fabs(w))` elevation. See SC2_PARTICLE_RE.md §17.8.
    ...
    case SpawnShape::Disc: {
        ...
        const f32 z = in.shape == SpawnShape::Cylinder
    ...
    case SpawnShape::Mesh: {
        if (in.mesh == nullptr)
            return {0.0f, 0.0f, 0.0f};
        const MeshSample s = SampleMeshSurface(rng, *in.mesh);
        if (normal != nullptr)
            *normal = s.normal;
        return s.position;
    }
...
Vector3f SampleSpawnVelocity(renderer::sc2::Rng& rng, const SpawnVelInputs& in,
    ...
    const f32 speedOverlay = DrawOverlay(rng, in.speedOverlay, in.variation);
    const f32 yaw = in.spawnYaw +
                    DrawOverlay(rng, in.yawOverlay, in.variation);
    const f32 pitch = in.spawnPitch +
                      DrawOverlay(rng, in.pitchOverlay, in.variation);
    const f32 horizontal =
        in.spawnHorizontal +
        DrawOverlay(rng, in.horizontalOverlay, in.variation);
    const f32 vertical =
        in.spawnVertical + DrawOverlay(rng, in.verticalOverlay, in.variation);
    ...
    switch (static_cast<VelocityType>(in.velocityType)) {
    case VelocityType::Cone: {
    ...
    case VelocityType::Radial:
        ...
        speed = drawSpeed();
        break;
    case VelocityType::Axis:
        dir = {0.0f, 0.0f, position.z >= 0.0f ? 1.0f : -1.0f};
        speed = drawSpeed();
        break;
    case VelocityType::Random: {
...
using renderer::sc2::kMidTimeCollapse;
...
std::array<u32, 3> SampleColor(renderer::sc2::Rng& rng, const ColorInputs& in) {
    const f32 overlay =
        DrawOverlay(rng, in.alphaOverlay, in.variation);
...
std::array<f32, 4> SampleSize(renderer::sc2::Rng& rng, const SizeInputs& in, f32 blend) {
    const f32 overlay = DrawOverlay(rng, in.sizeOverlay, in.variation);
    ...
    out[3] =
        InstanceTypeOf(in.instanceType) == InstanceType::Pinned ? in.instanceDistance : 1.0f;
    return out;
}

std::array<f32, 3> SampleRotation(renderer::sc2::Rng& rng, const RotationInputs& in) {
    const f32 overlay =
        DrawOverlay(rng, in.rotationOverlay, in.variation);
...
using renderer::sc2::kOrientQuant;
using renderer::sc2::kRotationQuant;
using renderer::sc2::kSizeQuant;
using renderer::sc2::kStillSpeedSq;
...
renderer::sc2::vs::Mat3 Upper3(const Matrix44f& b) {
    renderer::sc2::vs::Mat3 r{};
    for (usize i = 0; i < 3; ++i)
        for (usize j = 0; j < 3; ++j)
            r.m[i][j] = b.data[i][j];
    return r;
...
void InitSpawned(renderer::sc2::Rng& rng, const InitInputs& in,
    ...
    const bool emitterBasis =
        in.shape.shape == SpawnShape::Mesh || in.slot == 0;
    ...
    const renderer::sc2::vs::Mat3 basis3 = Upper3(basis);
    const renderer::sc2::vs::Mat3 nbasis3 = Upper3(nbasis);
    ...
    const u16 collideBits = static_cast<u16>(
        (in.parFlags * 2) & (renderer::sc2::kElemCollideTerrain | renderer::sc2::kElemCollideObjects));
    const bool noise = (in.emitFlagsWord & renderer::sc2::kEmitNoise) != 0;
    ...
    const bool meshNormal =
        static_cast<VelocityType>(in.velocity.velocityType) == VelocityType::MeshNormal;
    const bool lifespanRandom =
        Has(in.additionalFlags, ParticleAdditionalFlag::LifespanRandomize);
    const bool randomUv = Has(in.rotationFlags, RotationBit::RandomUvOffset);
    const bool massRandom = Has(in.additionalFlags, ParticleAdditionalFlag::MassRandomize);
    ...
    const renderer::sc2::vs::Mat3 local3 = Upper3(localXform);

    for (std::size_t n = 0; n < out.size(); ++n) {
        SpawnedElement& e = out[n];
        e = SpawnedElement{};
        ...
        Vector3f pos = SampleSpawnPosition(rng, in.shape, meshNormal ? &normal : nullptr);
        Vector3f velocity = SampleSpawnVelocity(rng, in.velocity, pos, normal);
        ...
        e.colorNodes = SampleColor(rng, in.color);
        const auto size = SampleSize(rng, in.size, sizeBlend);
        for (std::size_t k = 0; k < 4; ++k)
            e.size[k] = Quant(size[k] * kSizeQuant);
        const auto rot = SampleRotation(rng, in.rotation);
        for (std::size_t k = 0; k < 3; ++k)
        ...
        if (worldArm) {
            const Vector3f b = renderer::sc2::vs::MulVecMat3(pos, basis3);
            pos = {b.x + emitPos.x, b.y + emitPos.y, b.z + emitPos.z};
            if ((in.stateFlags & renderer::sc2::kStateInheritVelocity) != 0) {
                ...
                const Vector3f nv = renderer::sc2::vs::MulVecMat3(velocity, nbasis3);
                ...
                velocity = renderer::sc2::vs::MulVecMat3(velocity, basis3);
            }
            ...
            if (type == InstanceType::EmitterOriented)
                e.orientVec = packed;
            else if (type == InstanceType::TerrainDirOriented)
                e.orientVec = fwd;
        } else {
            ...
            if (type == InstanceType::EmitterOriented)
                e.orientVec = fwd;
            if (in.slot > 0 && in.hasBone) {
                const Vector3f p = renderer::sc2::vs::MulVecMat3(pos, local3);
                pos = {p.x + localXform.data[3][0], p.y + localXform.data[3][1],
                       p.z + localXform.data[3][2]};
                velocity = renderer::sc2::vs::MulVecMat3(velocity, local3);
            }
        }
        ...
        if (Has(in.parFlags, ParticleFlag::RandomFlipbookStart) && in.flipbookColumns != 0 &&
            in.flipbookRows != 0) {
        ...
        if (Has(in.parFlags, ParticleFlag::SpawnTrailingParticles) && in.hasChildEmitter1) {
            if (rng.RangeF(0.0f, 1.0f) <= in.trailChance) {
                e.flags |= renderer::sc2::kElemTrail;
                e.trailAccum = 0.0f;
            }
        }
        ...
        const u32 ms = in.nowMs + static_cast<u32>(static_cast<i32>(
                                      (e.deathTime - e.birthTime) * renderer::sc2::kMsPerSec));
