...
// out of each row — the emitter's orientation and nothing else.
//
// Kept as a matrix rather than converted to a quaternion on purpose. The client
// stores a quaternion here (`C4Quaternion::FromRotationMatrix` in
// `CreateParticle` @0x1016a08e0) only because it has to integrate the tumble into
// it every step; we integrate in closed form, so there is nothing to store, and
// extracting a quaternion from a basis is exactly where a row-vector convention
// silently inverts into a column-vector one.
...
void ModelParticleEmitter::OnParticleBorn(u32 poolIndex) {
    ...
    // (@0x1016a0c22..0x1016a0ccf against `InitializeLoaded` @0x100f57e30, which
    // fills the pairs (min, max-min) per axis). Reproduced, not corrected — a
...
Matrix44f ModelParticleEmitter::TransformFor(u32 poolIndex) const {
    ...
    // Then the emitter's own orientation, applied last because it is what maps
    // the particle's body frame into the world: frozen at birth for a
    // world-space emitter, read live for a model-space one that still rides it.
    //
    // `RenderParticle` post-multiplies the whole emitter matrix, scale and
    // translation included. Ours differs in exactly that, and it has to: the
    // child actor applies its own `worldScale`, which is the same uniform factor
    // the emitter matrix carries, and its translation is already in `pos`. Take
