...
void Emitter::RefreshGroundNormal(ParticleState& st, const Vector3f& pos) const {
    ...
    // How far up and down to look for ground: wide, as the engine's raycast has
    // no window (§30.13). A host wanting a tighter one answers false, which
    // lands on the miss default.
...
bool Emitter::SnapToGround(Vector3f& pos, f32 height) const {
    if (!surface_.groundQuery)
        return true;
    // The engine's lookup (@0x71000D3A00) walks the scenes under the XY and has
    // no vertical window, so neither does this.
    constexpr f32 kUnbounded = std::numeric_limits<f32>::infinity();
    f32 z = 0.0f;
    if (!surface_.groundQuery({pos.x, pos.y, pos.z - height}, kUnbounded, kUnbounded, z))
        return false;
    pos.z = height + z;
    return true;
}

bool Emitter::PlaceOnGround(Vector3f& pos) const {
    return !d3desc_->Has(PrtFlag::PlaceOnGround) || SnapToGround(pos, 0.0f);
}
...
void StepOrientation(const EmitterDesc& d, ParticleState& st, const ChannelSampler& ch, f32 dt,
    ...
    if (d.Cap(kCapSpin)) {
        ...
        // The turn goes on the LEFT, a world-space spin, in @0x71000BF178's sum
        // order, and runs even at zero rate.
        const f32 half = (w * dt) * 0.5f;
        const f32 s = std::sin(half);
        const Quaternion r{s * st.spinAxis.x, s * st.spinAxis.y, s * st.spinAxis.z, std::cos(half)};
        const Quaternion q = st.orientation;
        const Quaternion t{((r.y * q.z - r.z * q.y) + r.x * q.w) + r.w * q.x,
                           r.w * q.y + (r.y * q.w + (r.z * q.x - r.x * q.z)),
                           r.w * q.z + (r.z * q.w + (r.x * q.y - r.y * q.x)),
                           ((r.w * q.w - r.x * q.x) - r.y * q.y) - r.z * q.z};
        const f32 inv = 1.0f / std::sqrt(t.w * t.w + (t.z * t.z + (t.x * t.x + t.y * t.y)));
        st.orientation = {t.x * inv, inv * t.y, inv * t.z, inv * t.w};
    }
}
...
void StepAppearance(ParticleState& st, const ChannelSampler& ch, const EmitterFrame& f) {
    ...
    // ch6 is NOT this colour's alpha: it becomes COLOR1 (`MOV W9,#0x1010101`
    // @0x71000BEF20 -> particle+0xEC, forwarded @0x71000BCDC0), read only by
    // `ps_legacy`'s erosion tail. See §29.5.
...
bool Emitter::StepParticle(u32 idx, f32 dt, const EmitterFrame& f) {
    ...
    // Bit 13 stands the particle on the ground, raised by the Z its two offset
    // channels have reached; over no ground it dies.
    if (d.Has(PrtFlag::FollowGround)) {
        f32 height = d.Cap(kCapTripleA) ? st.prevOffsetA.z : 0.0f;
        if (d.Cap(kCapTripleB))
            height = height + st.prevOffsetB.z;
        if (!SnapToGround(p.position, height * u))
            return false;
    }
