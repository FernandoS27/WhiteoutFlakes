...
///
/// Every state belongs to the PARTICLE (§27.3). The draws come off the particle's
/// own stream, not the engine's global one, in the engine's ORDER (§27.7, §30.10).
...
///
/// Mode 2 integrates the scroll and WRAPS it into [0,1], or CLAMPS it with
/// `tAnim3.flAmount` set; the angle is clamped to 8x2pi and folded. Mode 3 steps
/// its flip-book over `count - 0.0001`; a once-shot clamps and zeroes its rate.
