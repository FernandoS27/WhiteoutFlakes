...
// ============================================================================
// The forty D3 particle channels, by ENGINE CHANNEL ID: argument 3 of every
// `InterpolationPath_Eval*` call, and the only key stable between the shipped
// struct version 180 and the Switch binary's 213. How the slot -> id tables were
// read is D3_PARTICLE_DESIGN.md §30.12.
// ============================================================================
...
/// `dwPrtFlags` — the bits the shipped runtime reads.
enum class PrtFlag : u32 {
    /// Bit 0: the system runs until told to stop — time mode 1, no release test.
    Persistent = 0x1u,
    /// Bit 2: a newborn's Z is the ground's under it, and a birth over no
    /// ground is dropped (`Particle_InitLifeAndSize` @0x71000B6D08). 538 files.
    PlaceOnGround = 0x4u,
    ...
    RandomRoll = 0x1000u,
    /// Bit 13: every step puts the particle back on the ground, raised by its
    /// two offset channels' Z, and kills it over no ground
    /// (`ParticleSystem_UpdateParticles` @0x71000BEA84). 362 files.
    FollowGround = 0x2000u,
...
/// The per-system capability mask `ParticleSystem_Spawn` derives at load and the
/// per-frame step branches on — not `dwPrtFlags` (§9, §9.1, §30.12). The
/// engine's extra bit 0x2000 (types 1, 3, 4) is answered by `EmitsActors`.
enum Capability : u32 {
    kCapOrbit = 0x0001,
    kCapRadial = 0x0002,
    kCapTripleA = 0x0004,
