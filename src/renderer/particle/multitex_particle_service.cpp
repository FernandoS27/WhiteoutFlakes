...
void MultiTexParticleService::Shutdown() {
    ...
    for (gfx::BufferHandle* b : {&vb_, &vsCb_, &psCb_}) {
...
bool MultiTexParticleService::BeginFrame(const MultiTexGeometry& geo,
    ...
    if (vb_ == gfx::BufferHandle::Invalid || count > vbCapacity_) {
        ...
        gfx::BufferDesc bd;
        bd.size = static_cast<u64>(sizeof(MultiTexVertex)) * static_cast<u64>(newSize);
    ...
    if (!mapped)
        return false;
    auto* dst = static_cast<MultiTexVertex*>(mapped);
...
void MultiTexParticleService::Draw(gfx::IGFXCommandList* cmd, const EmitterDrawList& dl,
    ...
    cmd->BindVertexBuffer(0, vb_, sizeof(MultiTexVertex));
