...
struct BuildGeometryInput {
    ...
    /// @brief Where a refraction or multi-texture emitter's two extra UV sets go:
    ///        `(uv1.xy, uv2.xy)` per vertex, in lockstep with @p out. Null for an
    ///        ordinary emitter; a layered one built with it null still makes its
    ///        quads, just not shadeable as layers. See M2_REFRACTION_DESIGN.md
    ///        and M2_MULTITEX_DESIGN.md.
