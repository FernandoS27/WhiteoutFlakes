...
// ===========================================================================
// WoW builder (`IBuildVertices<CParticle2,0,CGxVertexPCT>` @0x1016af940).
// Separate from the WC3 builder, whose every vertex the L2 baselines pin, and
// unreachable from a WC3 emitter. Builds in world space from the camera basis
// where the client builds in view space. See M2_PARTICLE_DESIGN.md §9, §11.4.
// ===========================================================================
i32 BuildWowGeometry(const Emitter2& emitter, const BuildGeometryInput& in,
    ...
    if (d.sortZ && emitter.Behavior().sortedBuilderDefect && order.size() > 1) {
        // The shipped defect: ranks 2,3,...,N,N (`top()` read after `pop()`).
        // Reproduce, do not repair. M2_PARTICLE_DESIGN.md `sortedBuilderDefect`, §11.4.
    ...
    // Refraction and multi-texture only: the second and third UV sets, one per
    // vertex alongside `out`: `uvN = particleUV[N] + corner01 * multiTexScale[N]`,
    // the corner being the sprite cell's own 0/1 pair. M2_PARTICLE_DESIGN.md §11.4.
    ...
    for (const DrawRecord& rec : order) {
        ...
        if (d.velocityOrient && velLenSq > kVelocityEpsilon) {
            ...
            // XYQuad lies the sprite flat in the WORLD XY plane, spinning about
            // its normal (`s_quadToView` is frame-global; M2_PARTICLE_DESIGN.md §9).
...
// The twinkle table (`CParticleEmitter2::Init` @0x10169efe0): uniform [0,1), but
// seeded fixed where the client uses `rand()` — this file's one deliberate
// divergence. See M2_PARTICLE_DESIGN.md §9.
