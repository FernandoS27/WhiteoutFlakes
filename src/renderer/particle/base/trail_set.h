...
/// The animated state a trail runs on: only the fields `Emitter2::ApplyState`
/// reads. A trail has no tracks, so its record's constant state is re-applied
/// every frame with the OWNING actor's four fields patched in. See
/// M2_PARTICLE_DESIGN.md §11.8.
struct TrailState {
