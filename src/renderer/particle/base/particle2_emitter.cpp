...
Emitter2::~Emitter2() {
    if (sc2_ && sc2PendingList_) {
        std::erase_if(*sc2PendingList_, [rt = sc2_.get()](const sc2::PendingModel& entry) {
...
void Emitter2::SetDesc(std::shared_ptr<const EmitterDesc> desc) {
    // A shapeless WC3/M2 desc is replaced: `CreateParticle` dereferences
    // `desc_->shape`. An SC2 desc has none (its shape is the number
    // `sc2.emit.shape`) and never reaches `EmitStep`. See M2_PARTICLE_DESIGN.md §11.1.
...
i32 Emitter2::TotalAlive() const {
    // An SC2 emitter keeps its elements in `sc2::ParticleStore`, never in `pool_`.
    if (sc2_)
...
void Emitter2::Sync() {
    ...
    // describe (`sc2::Runtime::Arm`) — not from `1.15 * rate * lifespan`, which
...
void Emitter2::SetSeed(u32 seed) {
    ...
    // An SC2 emitter draws from its runtime's `renderer::sc2::Rng`, which this seeds too,
...
void Emitter2::EmitStep(f32 elapsed, f32 emissionScaler) {
    ...
    if (enabled) {
        ...
        // Drain what was planned, not what the pool served: `EmitNewParticles`
        // @0x1016a5c90 drops an unserved spawn, so the carry stays below 1.
        // See M2_PARTICLE_DESIGN.md §11.1.
        ...
        while (planned > 0 && !pool_.DeadEmpty()) {
            if (behavior_.emitAlongPath) {
                if (desc_->randomEmissionSpacing) {
                    // The client draws a random point and spawns at the emitter
                    // anyway (`EmitNewParticles` @0x1016a5c90). The dead draw
                    // still advances the stream: keep it. M2_PARTICLE_DESIGN.md §11.1.
...
void Emitter2::AdvanceStep(f32 elapsed, f32 emissionScaler) {
    ...
    for (usize i = 0; i < pool_.AliveCount();) {
        ...
        // Under WC3 the aux word is a curve-segment cursor (a search hint for the
        // builder); under WoW it holds the lifespan variance and render seed, so
        // it must not advance. Death is age against lifespan only (M2_PARTICLE_DESIGN.md §11.1).
        if (!behavior_.birthDrawsVarianceAndSeed) {
...
void Emitter2::TickEmitterVelocity(f32 dt) {
    ...
    // zeroes it, so only a burst's onset inherits motion (M2_PARTICLE_DESIGN.md Q4).
...
i32 Emitter2::BuildGeometry(const BuildGeometryInput& in, std::vector<Vertex>& out) const {
    if (sc2_)
        return BuildGeometrySc2(*this, in, out);
