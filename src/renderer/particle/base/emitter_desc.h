...
// ============================================================================
// EmitterDesc — the immutable half of an emitter: fixed at load, built once per
// ModelTemplate and shared by every actor. The animated state (rate, speed,
// cone, plane size, visibility, transform), pool and RNG live on the emitter.
// Every format's adapter fills this same struct; nothing downstream knows which.
// ============================================================================
...
struct EmitterDesc {
    ...
    /// one multiplier — shifts this particle's render stream, nothing else's.
    ...
    /// @brief M2 `Refraction`: a screen-space distortion, not colour, drawn only in
    ///        its own pass (`AddParticleElement` @0x100f78590 → M2PASS_REFRACTION),
    ///        with the two extra scrolling UV sets below. See M2_REFRACTION_DESIGN.md.
    bool refraction = false;

    /// @brief M2 `MultiTexture`: three textures through three UV sets. Stays in the
    ///        transparent pass; changes the shader and vertex stream. See
    ///        M2_MULTITEX_DESIGN.md and M2_PARTICLE_DESIGN.md §11.3.
