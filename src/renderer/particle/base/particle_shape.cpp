...
// ---------------------------------------------------------------------------
// WoW generators. Draw order is load-bearing and matches the decompiles:
// position, then speed, then the two angles — which zSource mode SKIPS, moving
// the stream position of everything after.
// ---------------------------------------------------------------------------
...
void WowBoneShape::Sample(SpawnSample& out, const SpawnParams& p, RndSeed& rnd) const {
    ...
    // Straight up when zSource is off (no angle ranges to fall back on). The
    // zSource aim uses the fresh position, not the client's stale pool slot:
    // M2_PARTICLE_DESIGN.md §11.5.
