...
void TrailSet::Drive(const Emitter2& owner, const Particle2& p, f32 dt, f32 emissionScaler) {
    ...
    for (auto& t : trails_) {
        // Spawn from the owner's matrix with the particle's position as its
        // translation, then restore: the trail DRAWS with the parent's
        // (`UpdateLiveParticle` @0x1016aa080). M2_PARTICLE_DESIGN.md §11.8.
        ...
        // prev := cur, collapsing the spawn segment to the particle: the client
        // leaves prev uninitialised, which is not a behaviour to reproduce.
        // See M2_TRAIL_EMITTER_DESIGN.md D1.
