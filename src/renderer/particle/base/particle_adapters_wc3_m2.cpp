...
std::shared_ptr<const EmitterDesc> DescFromM2Config(const M2ParticleEmitterConfig& cfg,
    ...
    // Squirt is one mechanism in both clients; the rising edge is detected at the
    // actor layer (ApplyParticleFrameStates). M2_PARTICLE_DESIGN.md Q5 and §11.9.
    desc->emission.squirtAtStart = cfg.squirt;
...
DescFromWc3ChildModelConfig(const model::PE1EmitterConfig& cfg) {
    ...
    // starts with. No curves: death is age against lifespan alone.
    desc->longitude = 0.0f;
