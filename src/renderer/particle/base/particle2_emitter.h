...
#include "renderer/particle/base/trail_set.h"
#include "renderer/particle/base/wow_runtime.h"
...
// One concrete emitter, by composition: the shared desc carries the spawn shape,
// so a sphere or cone emitter is a different desc rather than a different class.
...
public:
    ...
    // No-ops while `sc2_` is null. On the base, like SetDesc and ApplyState, so
    // the service, actor layer and loader never know the family (design §3.4).
    ...
    void QueueSpawnRequest(const sc2::SpawnRequest& req);
    ...
    void DrainSpawnRequests(std::vector<sc2::RoutedSpawnRequest>& out);
    ...
    /// The model's active sequence this frame (`sc2::ActiveSequence`). A change
    ...
    void SetActiveSequenceSc2(i32 sequence);
    ...
    void AttachPendingListSc2(sc2::PendingModels* list);
    ...
    /// the SC2 arm is @ref BuildGeometrySc2.
    ...
    static i32 BuildGeometrySc2(const Emitter2& e, const BuildGeometryInput& in,
                                std::vector<Vertex>& out);

    /// The SC2 arm of `InternalUpdate`. Assembles the frame `sc2::TickEmitter`
...
protected:
    ...
    const sc2::Runtime* StateSc2() const {
        return sc2_.get();
    }
...
private:
    ...
    // Whether a spawn is displaced along the emitter's path this frame: never for
    // random spacing (see EmitStep) or model space. M2_PARTICLE_DESIGN.md §11.2.
    ...
    void RefreshMeshTrianglesSc2();
    void ApplyFrameSc2(const model::FrameState::ParticleFrameState::Sc2ParticleFrame& frame);
    ...
    std::unique_ptr<sc2::Runtime> sc2_;
    ...
    sc2::PendingModels* sc2PendingList_ = nullptr;

    /// Reset whole by a rewind.
    Wc3RunState run_;
    /// Adopted as another emitter's trail (M2 RPID): it never emits from its own
    /// position and its enable is sticky. M2_PARTICLE_DESIGN.md §11.2.
    bool isTrail_ = false;
    ...
    // Renderer units -> the space this emitter's particles are stored in. Emitter
    // motion (travel, inherited velocity, follow delta) arrives in renderer units;
    // a model-space particle lives in model units. M2_PARTICLE_DESIGN.md §11.2.
