...
// pose (OP14) and the pending-spawn draw (OP14b).
//
// Split from `particle_stages_sc2.cpp` for size, not for layering: the
// declarations live in `particle_stages_sc2.h` with every other kernel, and the
// contract is the same — the float operation ORDER is the binary's, written out
// with explicit parentheses because `a * b * c` would be a different number.
// ============================================================================
...
Sc2MeshSample Sc2SampleMeshSurface(sc2::Rng& rng, const Sc2MeshSurfaceInputs& in) {
    ...
    for (;;) {
        ...
        if (count == 0)
            return out;
        const Sc2MeshTriangle& tri = in.triangles[rng.RangeInt(0, count)];
        const Sc2MeshRegionBase base =
            tri.region < in.regions.size() ? in.regions[tri.region] : Sc2MeshRegionBase{};
...
const std::array<std::array<f32, 4>, 7> kSc2ModelOrientPresets = {{
...
Sc2ModelPose Sc2ModelParticlePose(const Sc2ModelPoseInputs& in) {
    ...
    if (in.legacyOrient) {
        ...
        if (index >= 0)
            preset = kSc2ModelOrientPresets[static_cast<usize>(index)];
    }
