...
Vector3f MulPoint(const Matrix44f& m, const Vector3f& v) {
    const f32 x = v.x * m.data[0][0] + v.y * m.data[1][0]
                + v.z * m.data[2][0] +        m.data[3][0];
    const f32 y = v.x * m.data[0][1] + v.y * m.data[1][1]
                + v.z * m.data[2][1] +        m.data[3][1];
    const f32 z = v.x * m.data[0][2] + v.y * m.data[1][2]
                + v.z * m.data[2][2] +        m.data[3][2];
    const f32 w = v.x * m.data[0][3] + v.y * m.data[1][3]
...
Matrix44f ShadowService::BuildLightViewLH(const Vector3f& center,
    ...
    else { const f32 invLen = 1.0f / std::sqrt(n2); L = { L.x*invLen, L.y*invLen, L.z*invLen }; }
...
Matrix44f ShadowService::OrthoLHOffcenter(f32 l, f32 r, f32 b, f32 t,
                                          f32 n, f32 f) {
