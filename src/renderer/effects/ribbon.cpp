#include "renderer/effects/ribbon.h"
#include "sim_util.h"
...
bool RibbonSystem::HasEmitters() const {
...
void RibbonSystem::Simulate(f32 dt) {
    ...
    for (auto& [id, em] : emitters_) {
        ...
        if (lifeSpan < kRibbonMinLifespan)
            lifeSpan = kRibbonMinLifespan;
        ...
        em.segments.erase(em.segments.begin(), std::find_if(em.segments.begin(), em.segments.end(),
                                                            [lifeSpan, dt](const RibbonSegment& s) {
                                                                return s.age < (lifeSpan - dt);
                                                            }));

        bool emittedHead = false;
        if (dt > 0 && IsEmitterVisible(em.state.visibility) && em.config.emission > 0 &&
            em.posSet) {
        ...
        usize updateEnd = emittedHead ? em.segments.size() - 1 : em.segments.size();
        for (usize i = 0; i < updateEnd; ++i) {
            auto& seg = em.segments[i];

            f32 dz = em.config.gravity * dt * dt + 2.0f * em.config.gravity * seg.age * dt;
            seg.top.z -= dz;
            seg.bot.z -= dz;
            seg.age += dt;
        }
...
RibbonSystem::StripResult RibbonSystem::BuildStrips() const {
    ...
    for (auto& [id, em] : emitters_) {
        if (em.state.visibility <= 0.0f)
            continue;
        if (em.segments.size() < 2)
            continue;
        ...
        Vector4f vertColor = {em.state.color.x, em.state.color.y, em.state.color.z, em.state.alpha};
