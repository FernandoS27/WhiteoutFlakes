...
public:
    CornEffectsEmitter(CornEffectsAssetCache& cache,
                    std::string pkbPath,
    ...
    void SetScale(f32 s)                     { hostScale_ = s; }
    void SetEmissionRateMultiplier(f32 v)    { emissionRateMultiplier_ = v; }
    void SetLifeSpanMultiplier(f32 v)        { lifeSpanMultiplier_     = v; }
    void SetSpeedMultiplier(f32 v)           { speedMultiplier_        = v; }
    void SetColor(const Vector4f& rgba)      { color_ = rgba; }
    void SetWeatherParams(const Vector3f& position,
                          const Vector2f& size,
                          f32 emissionRate);
    void SetReplaceableId(i32 id)            { replaceableId_ = id; }
    void SetReplaceableColor(const Vector4f& rgba) { replaceableColor_ = rgba; }
    void SetOwningAgentVisibility(bool visible);
    void SetCurrentAnimationName(const char* name);

    void Update(f32 dt, bool paused);

    void MarkRenderAttempted() { simFlags_ |= SimFlag_RenderAttempted; }

    bool Alive() const;
    i32  TotalAlive() const;
    const std::string& PkbPath() const { return pkbPath_; }
    ::whiteout::cornflakes::EffectRuntime* LiveRuntime() const { return live_.runtime.get(); }

    void SetBackendInit(const std::optional<CornEffectsGfxBackend::Init>& init) {
        backendInit_ = init;
    }
    void SetFrameInputs(const CornEffectsFrameInputs& f) { frameInputs_ = f; }
...
private:
    ...
    f32 GetCornFxScale() const { return cornEffectsScaling_ ? hostScale_ : 1.0f; }
