...
PsoTraceEntry MakeEntry(const PsoRequest& r) {
    ...
    e.flags     = (r.wireframe   ? 0x1u : 0u)
                | (r.lhClipSpace ? 0x2u : 0u);
...
void BlsPsoTrace::Load() {
    ...
    if (!f || magic != kMagic || version != kVersion) return;

    // Sanity cap so a corrupt header doesn't drive a multi-GB allocation.
    if (count > 1'000'000u) return;
    ...
    keys_.reserve(count);
    for (const auto& e : entries_) keys_.insert(EntryKey(e));
}
...
void BlsPsoTrace::Replay(BlsPsoBuilder& builder,
                          const BlsProgramCatalog& catalog) const {
