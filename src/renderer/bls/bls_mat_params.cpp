...
GxMatAlpha FilterToGxAlpha(i32 filterMode) {
    switch (filterMode) {
    case FILTER_NONE:
        return GxMatAlpha::Opaque;
    case FILTER_TRANSPARENT:
        return GxMatAlpha::AlphaKey;
    case FILTER_BLEND:
        return GxMatAlpha::Blend;
    case FILTER_ADDITIVE:
    case FILTER_ADD_ALPHA:
        return GxMatAlpha::Add;
    case FILTER_MODULATE:
        return GxMatAlpha::Modulate;
    case FILTER_MODULATE_2X:
        return GxMatAlpha::Modulate2X;
    default:
        return GxMatAlpha::Opaque;
    }
}
...
u32 FilterToDisables(i32 filterMode) {
    ...
    case FILTER_TRANSPARENT:
        return 0;
    case FILTER_BLEND:
        return kDisableDepthWrite;
    case FILTER_ADDITIVE:
    case FILTER_ADD_ALPHA:
    case FILTER_MODULATE:
    case FILTER_MODULATE_2X:
        return kDisableFog | kDisableDepthWrite;
    default:
        return 0;
    }
}
...
MatParams FromMdxLayer(i32 filterMode, i32 matFlags, GxShaderID shaderId) {
    ...
    if (matFlags & MAT_TWO_SIDED)
        p.disables |= kDisableCull;
    if (matFlags & MAT_UNSHADED)
        p.disables |= kDisableLighting;
    if (matFlags & MAT_UNFOGGED)
        p.disables |= kDisableFog;
    if (matFlags & MAT_NO_DEPTH_TEST)
        p.disables |= kDisableDepthTest;
    if (matFlags & MAT_NO_DEPTH_SET)
...
MatParams FromParticleDesc(const particle::ParticleMaterialDesc& desc, GxShaderID shaderId) {
    MatParams p;
    p.shaderId = shaderId;
