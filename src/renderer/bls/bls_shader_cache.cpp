...
BlsShader* BlsShaderCache::Acquire(gfx::ShaderStage stage, const std::string& name) {
    ...
    // path. Vulkan / WebGPU / Metal can't fall back: SPIR-V/WGSL/MTLB
    // and DXBC aren't interchangeable.
    ...
    //   Metal  → MTLB-in-v1.8 under <root>/metal/<stage>/
    ...
    auto tryRoot = [&](const std::string& root) {
        ...
        if (api_ == gfx::GfxApi::Metal)
            return tryRead(root, "metal/");
