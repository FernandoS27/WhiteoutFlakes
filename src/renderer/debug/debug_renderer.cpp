...
void DebugRenderer::RenderCollisions() {
    ...
    for (auto& cs : shapes) {
        ...
        if (cs.type == 0) {
            ...
            i32 edges[24] = {0,1, 1,2, 2,3, 3,0, 4,5, 5,6, 6,7, 7,4, 0,4, 1,5, 2,6, 3,7};
            ...
            f32 axisLen = std::sqrt(axisVec.x*axisVec.x + axisVec.y*axisVec.y + axisVec.z*axisVec.z);
...
void DebugRenderer::RenderViewCube() {
    ...
    Matrix44f vcProj = Matrix44f::perspective_fov_rh(std::numbers::pi_v<f32> / 4.0f, 1.0f, 0.1f, 100.0f);
    ...
    if (vcHovered_) {
        gfx::Viewport homeVp = {
            vp.x + (f32)s * kViewCubeHomeOffset,
            vp.y - 24.0f,
            (f32)s * 0.3f, 20.0f,
            0.0f, 1.0f
        };
...
i32 DebugRenderer::HitTestViewCube(i32 mx, i32 my) const {
    ...
    for (i32 i = 0; i < 6; i++) {
        ...
        f32 cx = c.x*vp_mat.data[0][0] + c.y*vp_mat.data[1][0] + c.z*vp_mat.data[2][0] + vp_mat.data[3][0];
        f32 cy = c.x*vp_mat.data[0][1] + c.y*vp_mat.data[1][1] + c.z*vp_mat.data[2][1] + vp_mat.data[3][1];
        f32 cw = c.x*vp_mat.data[0][3] + c.y*vp_mat.data[1][3] + c.z*vp_mat.data[2][3] + vp_mat.data[3][3];
