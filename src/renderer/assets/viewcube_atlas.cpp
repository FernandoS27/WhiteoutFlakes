...
struct Label {
    const Glyph* glyphs[5];
    i32 count;
};

constexpr Label kLabels[kFaces] = {
    {{&G_F, &G_R, &G_O, &G_N, &G_T}, 5},       {{&G_B, &G_A, &G_C, &G_K, nullptr}, 4},
    {{&G_L, &G_E, &G_F, &G_T, nullptr}, 4},    {{&G_R, &G_I, &G_G, &G_H, &G_T}, 5},
    {{&G_T, &G_O, &G_P, nullptr, nullptr}, 3}, {{&G_B, &G_O, &G_T, nullptr, nullptr}, 3},
};

constexpr RGBA kFaceBg[kFaces] = {
    {100, 140, 190, 230}, {190, 120, 100, 230}, {100, 180, 120, 230},
    {190, 170, 100, 230}, {160, 160, 180, 230}, {140, 130, 120, 230},
};
...
std::vector<u8> GenerateViewCubeAtlas(i32& outW, i32& outH) {
    ...
    for (i32 face = 0; face < kFaces; ++face) {
        ...
        for (i32 i = 0; i < label.count; ++i) {
            DrawGlyph2x(pixels.data(), *label.glyphs[i], ox + i * (kGlyphW + kGap), oy, kText);
        }
    }
