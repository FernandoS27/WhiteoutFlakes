...
DncSample Sample(const DncAsset& asset, f32 todHours, f32 hoursPerDay, f32 ambModifier) {
    DncSample s;
    ...
    const i32 animMsRel =
        (animLen > 0) ? static_cast<i32>((wrapped * static_cast<f32>(animLen)) / hoursPerDay) : 0;
    const i32 animMs = asset.seqStartMs + animMsRel;

    const auto& L = asset.model.lights[0];

    std::vector<Matrix44f> boneWorld, allNodes;
    asset.hierarchy.Evaluate(animMs, asset.seqStartMs, asset.seqEndMs, asset.model.globalSequences,
                             boneWorld, allNodes, nullptr, animMs);
    ...
    if (L.colorTracks.isUsed) {
        Vector3f animated =
            EvaluateTrackVec3(L.colorTracks, animMs, asset.seqStartMs, asset.seqEndMs, L.color);
        color = {animated.z, animated.y, animated.x};
    }
    const f32 intensity =
        std::max(0.0f, EvaluateTrackF32(L.intensityTracks, animMs, asset.seqStartMs, asset.seqEndMs,
                                        L.intensity));
    ...
    if (L.ambientColorTracks.isUsed) {
        Vector3f animated = EvaluateTrackVec3(L.ambientColorTracks, animMs, asset.seqStartMs,
                                              asset.seqEndMs, L.ambientColor);
        ambColor = {animated.z, animated.y, animated.x};
    }
    const f32 ambI =
        std::max(0.0f, EvaluateTrackF32(L.ambientIntensityTracks, animMs, asset.seqStartMs,
    ...
    const f32 n2 =
        s.worldDir.x * s.worldDir.x + s.worldDir.y * s.worldDir.y + s.worldDir.z * s.worldDir.z;
    if (n2 > 1.0e-12f) {
