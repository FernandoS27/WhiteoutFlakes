...
public:
    ...
    ID3D12RootSignature* GetGraphicsRS() const {
        return graphicsRS_;
    }
    ID3D12RootSignature* GetComputeRS() const {
        return computeRS_;
    }

    DescriptorHeapRing& CbvSrvUavRing() {
        return cbvSrvUavRing_;
    }
    DescriptorHeapRing& SamplerRing() {
        return samplerRing_;
    }
    UploadRing& Upload() {
        return uploadRing_;
    ...
    BufferEntry* GetBuffer(BufferHandle h) {
        return buffers_.Get(static_cast<u64>(h));
    }
    ...
    SamplerEntry* GetSampler(SamplerHandle h) {
        return samplers_.Get(static_cast<u64>(h));
    }
    SwapChainEntry* GetSwapChain(SwapChainHandle h) {
        return swapChains_.Get(static_cast<u64>(h));
    }
