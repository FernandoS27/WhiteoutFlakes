...
bool UploadTexturePixels(VulkanDeviceState& state,
                          ...
                          const void* initialPixels)
{
    ...
    return SubmitTransferAndDeferStaging(state, stagingBuf, stagingAlloc,
        [&](vk::raii::CommandBuffer& cmdBuf) {
            ...
            cmdBuf.pipelineBarrier2({ .imageMemoryBarrierCount = 1, .pImageMemoryBarriers = &toRead });
        });
}
                                          ...
                                          const void* initialPixels)
{
    ...
    texture.aspect = isDepth ? (vk::ImageAspectFlagBits::eDepth |
                          (hasStencilAspect
...
TextureHandle CreateRenderTargetImage(VulkanDeviceState& state, i32 w, i32 h,
                                      vk::Format fmt, VkImageUsageFlags usage,
                                      vk::ImageAspectFlags aspect) {
    VkImageCreateInfo ici{ VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO };
    ici.imageType     = VK_IMAGE_TYPE_2D;
    ici.format        = static_cast<VkFormat>(fmt);
