...
inline bool SubmitTransferAndDeferStaging(VulkanDeviceState& state, VkBuffer stagingBuf,
                                          VmaAllocation stagingAlloc, RecordFn&& record) {
    ...
    state.pendingTransferDeletes.push_back(MakePendingDelete(
        signalValue,
        [stagingBuf, stagingAlloc, ownedCb = std::move(cmdBuf)](VulkanDeviceState& st) mutable {
