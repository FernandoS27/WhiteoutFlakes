...
public:
    ...
    // Load-op variant: the color attachment is bound with loadOp=Load, so
    // its existing contents are preserved. Depth is CLEARED by default —
    // pass `loadDepth` to keep it, which is what a pass that draws
    // depth-tested geometry into a side buffer needs (the WoW refraction
    // mask: its own colour target, the scene's depth). Default impl falls
    // back to the single-RT clearing variant for backends that haven't
    // implemented load-op yet — slot 0 still works correctly there because
    // the first frame's clear happens elsewhere; subsequent passes that
