...
Quaternion Wc3Slerp(const Quaternion& a, const Quaternion& b, f32 t) {
    ...
    if (d > DOT_THRESHOLD) {
        ...
        Quaternion result = Quaternion(a.x + t * (end.x - a.x), a.y + t * (end.y - a.y),
...
Quaternion Wc3Squad(const Quaternion& start, const Quaternion& outtan, const Quaternion& inttan,
                    const Quaternion& end, f32 t) {
...
static KeyBracket FindBracket(const u32* timestamps, i32 count, i32 timeMs, i32 seqStart,
                              i32 seqEnd) {
    KeyBracket b;
    if (count == 0)
        return b;
    ...
    if (timeMs < firstFrame || timeMs >= lastFrame) {
        ...
        if (segLen <= 0)
            return b;
        i32 pos =
            (timeMs >= lastFrame) ? timeMs - lastFrame : (timeMs - seqStart) + (seqEnd - lastFrame);
...
static Vector3f ApplyV3(ScalarFn fn, const Vector3f& a, const Vector3f& ota, const Vector3f& itb,
                        const Vector3f& b, f32 t) {
    return {fn(a.x, ota.x, itb.x, b.x, t), fn(a.y, ota.y, itb.y, b.y, t),
            fn(a.z, ota.z, itb.z, b.z, t)};
}

template <typename ScalarFn>
static Vector4f ApplyV4(ScalarFn fn, const Vector4f& a, const Vector4f& ota, const Vector4f& itb,
                        const Vector4f& b, f32 t) {
    return {fn(a.x, ota.x, itb.x, b.x, t), fn(a.y, ota.y, itb.y, b.y, t),
            fn(a.z, ota.z, itb.z, b.z, t), fn(a.w, ota.w, itb.w, b.w, t)};
}
...
static T EvaluateTrackImpl(const Track<T>& track, i32 timeMs, i32 seqStart, i32 seqEnd,
    ...
    const bool useCurve =
        (interp == InterpolationType::Hermite || interp == InterpolationType::Bezier);
    ...
    if (useCurve) {
        ...
        if (br.lo < 0)
            return defaultVal;
        if (br.lo == br.hi || forceNoInterp)
            return keys[br.lo].value;
        return curve(keys[br.lo].value, keys[br.lo].outTan, keys[br.hi].inTan, keys[br.hi].value,
                     br.t, interp);
    }
...
f32 EvaluateTrackF32(const Track<f32>& track, i32 timeMs, i32 seqStart, i32 seqEnd, f32 defaultVal,
                     bool forceNoInterp) {
    return EvaluateTrackImpl<f32>(
        track, timeMs, seqStart, seqEnd, defaultVal,
        [](f32 a, f32 b, f32 t) { return a + (b - a) * t; },
        [](f32 a, f32 ota, f32 itb, f32 b, f32 t, InterpolationType it) {
            return it == InterpolationType::Hermite ? HermiteInterp(a, ota, itb, b, t)
...
Vector3f EvaluateTrackVec3(const Track<Vector3f>& track, i32 timeMs, i32 seqStart, i32 seqEnd,
                           Vector3f defaultVal) {
    return EvaluateTrackImpl<Vector3f>(
        track, timeMs, seqStart, seqEnd, defaultVal,
        [](const Vector3f& a, const Vector3f& b, f32 t) { return Vector3f::lerp(a, b, t); },
        [](const Vector3f& a, const Vector3f& ota, const Vector3f& itb, const Vector3f& b, f32 t,
           InterpolationType it) {
            return it == InterpolationType::Hermite ? ApplyV3(HermiteInterp, a, ota, itb, b, t)
...
Vector4f EvaluateTrackVec4(const Track<Vector4f>& track, i32 timeMs, i32 seqStart, i32 seqEnd,
                           Vector4f defaultVal) {
    return EvaluateTrackImpl<Vector4f>(
        track, timeMs, seqStart, seqEnd, defaultVal,
        [](const Vector4f& a, const Vector4f& b, f32 t) { return Vector4f::lerp(a, b, t); },
        [](const Vector4f& a, const Vector4f& ota, const Vector4f& itb, const Vector4f& b, f32 t,
           InterpolationType it) {
            return it == InterpolationType::Hermite ? ApplyV4(HermiteInterp, a, ota, itb, b, t)
...
Quaternion EvaluateTrackQuat(const Track<Quaternion>& track, i32 timeMs, i32 seqStart, i32 seqEnd,
    ...
    return EvaluateTrackImpl<Quaternion>(
        track, timeMs, seqStart, seqEnd, defaultVal,
        [](const Quaternion& a, const Quaternion& b, f32 t) { return Wc3Slerp(a, b, t); },
        [](const Quaternion& a, const Quaternion& ota, const Quaternion& itb, const Quaternion& b,
           f32 t, InterpolationType) { return Wc3Squad(a, ota, itb, b, t); });
}

u32 EvaluateTrackU32(const Track<u32>& track, i32 timeMs, i32 seqStart, i32 seqEnd,
                     u32 defaultVal) {
    if (!track.isUsed || track.keyCount == 0)
        return defaultVal;
...
void MdxHierarchy::Build(const whiteout::mdx::Model& model) {
    ...
    for (i32 i = 0; i < (i32)nodes_.size(); i++) {
        ...
        if (n.source == HierarchyNode::Source::Bone && n.sourceIndex >= 0 &&
            n.sourceIndex < boneCount_) {
...
void MdxHierarchy::Evaluate(i32 timeMs, i32 seqStart, i32 seqEnd,
                            ...
                            std::vector<Matrix44f>& allNodeMatrices, const Vector3f* cameraPos,
    ...
    auto effTime = [&](const auto* track) -> EffTime {
        ...
        if (!track || !track->isUsed)
            return e;
        if (track->globalSequenceId == Track<f32>::kNoGlobalSequence)
            return e;
    ...
    for (i32 i = 0; i < nc; i++) {
        ...
        Matrix44f worldM = M;
        worldM.data[3][0] -= currPivot.x * worldM.data[0][0] + currPivot.y * worldM.data[1][0] +
                             currPivot.z * worldM.data[2][0];
        worldM.data[3][1] -= currPivot.x * worldM.data[0][1] + currPivot.y * worldM.data[1][1] +
                             currPivot.z * worldM.data[2][1];
        worldM.data[3][2] -= currPivot.x * worldM.data[0][2] + currPivot.y * worldM.data[1][2] +
                             currPivot.z * worldM.data[2][2];
    ...
    for (i32 i = 0; i < nc; i++) {
        if (nodes_[i].source == HierarchyNode::Source::Bone && nodes_[i].sourceIndex < boneCount_) {
...
i32 MdxHierarchy::BoneIndexToNodeIndex(i32 boneIdx) const {
    if (boneIdx < 0 || boneIdx >= (i32)boneIdxToNodeIdx_.size())
        return -1;
