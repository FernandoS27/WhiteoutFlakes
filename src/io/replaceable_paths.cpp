...
void LoadCliffTypes(IContentProvider& cp, DataCache& cache) {
    ...
    if (!data) {
        std::fprintf(stderr, "[events] ERR: CliffTypes.slk: not found\n");
        return;
    }
    ...
    for (usize r = 1; r < t.RowCount(); ++r) {
        ...
        if (id.empty() || dir.empty() || file.empty())
            continue;
...
void SetCurrentTileset(Tileset ts) {
    if (static_cast<u8>(ts) >= static_cast<u8>(Tileset::Count))
        return;
...
const char* ReplaceableCanonicalPath(i32 replaceableId, Tileset ts) {
    ...
    case 31:
        return "ReplaceableTextures\\LordaeronTree\\LordaeronSummerTree.blp";
    case 32:
        return "ReplaceableTextures\\AshenvaleTree\\AshenTree.blp";
    case 33:
        return "ReplaceableTextures\\BarrensTree\\BarrensTree.blp";
    case 34:
        return "ReplaceableTextures\\NorthrendTree\\NorthTree.blp";
    case 35:
        return "ReplaceableTextures\\Mushroom\\MushroomTree.blp";
    case 36:
        return "ReplaceableTextures\\RuinsTree\\RuinsTree.blp";
    case 37:
        return "ReplaceableTextures\\OutlandMushroomTree\\MushroomTree.blp";
