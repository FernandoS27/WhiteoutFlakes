...
void LoadSpawnData(IContentProvider& cp, DataCache& cache) {
    ...
    if (!data) {
        std::fprintf(stderr, "[events] ERR: SpawnData.slk: not found\n");
        return;
    }
...
void LoadSplatData(IContentProvider& cp, DataCache& cache) {
    ...
    if (!data) {
        std::fprintf(stderr, "[events] ERR: SplatData.slk: not found\n");
        return;
    }
    SlkTable t = ParseSlk(*data);
    ...
    const i32 cER = FindAny(t, {"EndR"}), cEG = FindAny(t, {"EndG"}), cEB = FindAny(t, {"EndB"}),
              cEA = FindAny(t, {"EndA"});
    ...
    for (usize r = 1; r < t.RowCount(); ++r) {
        ...
        if (cSR >= 0) {
            e.startC[0] = ParseFloat(t.Cell(r, cSR)) / 255.f;
            e.startC[1] = ParseFloat(t.Cell(r, cSG)) / 255.f;
            e.startC[2] = ParseFloat(t.Cell(r, cSB)) / 255.f;
            e.startC[3] = ParseFloat(t.Cell(r, cSA)) / 255.f;
        }
        if (cMR >= 0) {
            e.midC[0] = ParseFloat(t.Cell(r, cMR)) / 255.f;
            e.midC[1] = ParseFloat(t.Cell(r, cMG)) / 255.f;
            e.midC[2] = ParseFloat(t.Cell(r, cMB)) / 255.f;
            e.midC[3] = ParseFloat(t.Cell(r, cMA)) / 255.f;
        }
        if (cER >= 0) {
            e.endC[0] = ParseFloat(t.Cell(r, cER)) / 255.f;
            e.endC[1] = ParseFloat(t.Cell(r, cEG)) / 255.f;
            e.endC[2] = ParseFloat(t.Cell(r, cEB)) / 255.f;
            e.endC[3] = ParseFloat(t.Cell(r, cEA)) / 255.f;
        }
...
void LoadUberSplatData(IContentProvider& cp, DataCache& cache) {
    ...
    if (!data) {
        std::fprintf(stderr, "[events] ERR: UberSplatData.slk: not found\n");
        return;
    }
    SlkTable t = ParseSlk(*data);
    ...
    for (usize r = 1; r < t.RowCount(); ++r) {
        ...
        e.scale = (cScale >= 0) ? ParseFloat(t.Cell(r, cScale)) : 1.0f;
        if (cSR >= 0) {
            e.c[0][0] = ParseFloat(t.Cell(r, cSR)) / 255.f;
            e.c[0][1] = ParseFloat(t.Cell(r, cSG)) / 255.f;
            e.c[0][2] = ParseFloat(t.Cell(r, cSB)) / 255.f;
            e.c[0][3] = ParseFloat(t.Cell(r, cSA)) / 255.f;
        }
        if (cMR >= 0) {
            e.c[1][0] = ParseFloat(t.Cell(r, cMR)) / 255.f;
            e.c[1][1] = ParseFloat(t.Cell(r, cMG)) / 255.f;
            e.c[1][2] = ParseFloat(t.Cell(r, cMB)) / 255.f;
            e.c[1][3] = ParseFloat(t.Cell(r, cMA)) / 255.f;
        }
        if (cER >= 0) {
            e.c[2][0] = ParseFloat(t.Cell(r, cER)) / 255.f;
            e.c[2][1] = ParseFloat(t.Cell(r, cEG)) / 255.f;
            e.c[2][2] = ParseFloat(t.Cell(r, cEB)) / 255.f;
            e.c[2][3] = ParseFloat(t.Cell(r, cEA)) / 255.f;
        }
...
void LoadSoundSlk(IContentProvider& cp, DataCache& cache, const char* path) {
    ...
    for (usize r = 1; r < st.RowCount(); ++r) {
        ...
        if (code.empty() || code == "_")
            continue;
        SndEntry e;
        if (sFiles >= 0) {
            for (auto& tok : SplitComma(st.Cell(r, sFiles))) {
                auto it = cache.assetMap.find(ToLower(tok));
                e.filePaths.push_back(it != cache.assetMap.end() ? it->second : std::move(tok));
            }
        }
...
void LoadAllSoundSlks(IContentProvider& cp, DataCache& cache) {
    static constexpr const char* kAssetSlks[] = {
        "UI\\SoundInfo\\DialogueCreepsBase.slk",   "UI\\SoundInfo\\DialogueDemonBase.slk",
        "UI\\SoundInfo\\DialogueHumanBase.slk",    "UI\\SoundInfo\\DialogueNagaBase.slk",
    ...
    static constexpr const char* kSoundSlks[] = {
        ...
        "UI\\SoundInfo\\AnimSounds.slk",    "UI\\SoundInfo\\AbilitySounds.slk",
