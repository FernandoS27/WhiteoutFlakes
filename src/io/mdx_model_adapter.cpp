...
inline void swizPos(Vector3f& v) {
    v = {v.y, -v.x, v.z};
}
inline void swizScale(Vector3f& v) {
    v = {v.y, v.x, v.z};
}
inline void swizQuat(Quaternion& q) {
    q = {q.y, -q.x, q.z, q.w};
}
inline void swizTangent(Vector4f& t) {
    t = {t.y, -t.x, t.z, t.w};
}
...
void TransformMdxModelToMaxCoords(whiteout::mdx::Model& m) {
    ...
    for (auto& gs : m.geosets) {
        for (auto& v : gs.vertexPositions)
            swizPos(v);
        for (auto& v : gs.vertexNormals)
            swizPos(v);
        for (auto& t : gs.tangents)
            swizTangent(t);
    ...
    for (auto& bp : m.bindPoses)
        transformBindPose(bp);
}

inline const Layer::SubTexture* FindDiffuseSubTexture(const std::vector<Layer::SubTexture>& subs) {
    if (subs.empty())
        return nullptr;
    for (const auto& s : subs)
        if (s.slot == Layer::SlotType::DiffuseMap)
            return &s;
    return &subs[0];
}
...
MdxModelAdapter::MdxModelAdapter(whiteout::mdx::Model model, fs::path basePath,
    ...
    : model_(std::move(model)), basePath_(std::move(basePath)), resolver_(basePath_),
    ...
    for (usize i = 0; i < nodes.size(); ++i) {
        if (nodes[i].source != HierarchyNode::Source::Bone)
            continue;
        const auto& bone = model_.bones[nodes[i].sourceIndex];
        if (bone.geosetId == whiteout::mdx::Bone::MULTIPLE_GEOSETS)
            continue;
        const whiteout::u32 gaId = bone.geosetAnimationId;
        if (gaId == whiteout::mdx::Bone::MULTIPLE_GEOSETS)
            continue;
        if (gaId >= model_.geosetAnimations.size())
            continue;
        const whiteout::u32 targetGeoset = model_.geosetAnimations[gaId].geosetId;
        if (targetGeoset >= model_.geosets.size())
            continue;
...
std::vector<MeshData> MdxModelAdapter::GetMeshes() {
    ...
    for (i32 i = 0; i < (i32)model_.geosets.size(); i++) {
        ...
        const bool hasUv1 =
            gs.textureCoordinateSets.size() >= 2 && (i32)gs.textureCoordinateSets[1].size() == vc;
        if (hasUv1)
            mesh.uvs1.resize(vc);
        ...
        for (i32 v = 0; v < vc; v++) {
            ...
            if (!gs.textureCoordinateSets.empty() && v < (i32)gs.textureCoordinateSets[0].size()) {
                mesh.uvs[v] = {gs.textureCoordinateSets[0][v].x, gs.textureCoordinateSets[0][v].y};
            }
            if (hasUv1) {
                mesh.uvs1[v] = {gs.textureCoordinateSets[1][v].x, gs.textureCoordinateSets[1][v].y};
            }
            if (hasTangents)
...
i32 MdxModelAdapter::MapShadingFlags(Layer::ShadingFlag sf) const {
    ...
    const u32 s = (u32)sf;
    i32 flags = 0;
    if (hasFlag(s, SF::TwoSided))
        flags |= MAT_TWO_SIDED;
    if (hasFlag(s, SF::Unshaded))
        flags |= MAT_UNSHADED;
    if (hasFlag(s, SF::Unfogged))
        flags |= MAT_UNFOGGED;
    if (hasFlag(s, SF::NoDepthTest))
    ...
    if (hasFlag(s, SF::BackFacesForShadows))
        flags |= MAT_BACK_FACES_FOR_SHADOWS;
    return flags;
}
        ...
        for (i32 li = 0; li < (i32)mat.layers.size(); ++li) {
            ...
            if (!layer.subTextures.empty()) {
                ...
                auto subAt = [&](usize pos) -> i32 {
                    return pos < layer.subTextures.size() ? (i32)layer.subTextures[pos].textureId
                                                          : -1;
                };
...
SkeletonData MdxModelAdapter::GetSkeleton() {
    ...
    for (i32 i = 0; i < (i32)nodes.size(); i++) {
        ...
        sk.billboardFlags[i] =
            PackBillboardFlags(hasFlag(nf, NF::Billboarded), hasFlag(nf, NF::BillboardedLockX),
                               hasFlag(nf, NF::BillboardedLockY), hasFlag(nf, NF::BillboardedLockZ),
...
std::vector<SkinWeightData> MdxModelAdapter::GetSkinWeights() {
    ...
    auto resolveBoneIdx = [&](i32 matsValue) -> i32 {
        ...
        if (nodeIdx < 0)
            nodeIdx = hierarchy_.ObjectIdToNodeIndex(matsValue);
    ...
    for (i32 gi = 0; gi < (i32)model_.geosets.size(); gi++) {
        ...
        if (!gs.skinData.empty()) {
            ...
            for (i32 v = 0; v < vc && v < (i32)gs.vertexGroups.size(); v++) {
                ...
                if (groupId >= (i32)gs.matrixGroups.size())
                    continue;
                ...
                if (pseudoLocal >= 0) {
                    ...
                    for (u32 k = 0; k < count && (start + k) < gs.matrixIndices.size(); k++) {
                        ...
                        i32 localSlot =
                            (it != globalToLocal.end()) ? it->second : addToSubset(globalNode);
                        sw.influences[v].boneIdx[k] = localSlot;
        ...
        if (paletteSlotsUsed > kMaxPaletteSlots) {
            std::fprintf(stderr, "[GetSkinWeights] geoset %d needs %d palette slots (>%d)\n", gi,
                         paletteSlotsUsed, kMaxPaletteSlots);
        }
...
std::vector<CollisionShapeData> MdxModelAdapter::GetCollisionShapes() {
    ...
    for (const auto& cs : model_.collisionShapes) {
        ...
        if (cs.vertices.size() >= 1)
            cd.vertices[0] = cs.vertices[0];
        if (cs.vertices.size() >= 2)
            cd.vertices[1] = cs.vertices[1];
...
FrameState MdxModelAdapter::Evaluate(const ::whiteout::flakes::PoseRequest& req) const {
    ...
    hierarchy_.Evaluate(timeMs, seqStart, seqEnd, model_.globalSequences, boneWorld, allNodes,
                        &cameraPos, globalTimeMs);

    fs.boneWorldMatrices = std::move(allNodes);

    auto effectiveTime = [&](u32 gsId) -> std::tuple<i32, i32, i32> {
        if (gsId != whiteout::mdx::Track<whiteout::f32>::kNoGlobalSequence &&
            gsId < (u32)model_.globalSequences.size()) {
    ...
    auto nodeOf = [&](const Node& n) { return hierarchy_.ObjectIdToNodeIndex((i32)n.objectId); };
    ...
    for (usize ni = 0; ni < hierNodes.size(); ++ni) {
        u8 vis = (hierNodes[ni].parentIdx < 0) ? u8{1} : nodeVisible[hierNodes[ni].parentIdx];
        const i32 gate = boneGateGeoset_[ni];
        if (vis && gate >= 0 && fs.geosetAlphas[gate] <= 0.0f)
            vis = 0;
        nodeVisible[ni] = vis;
    }
    auto gateByBoneAncestors = [&](i32 nodeIdx) -> f32 {
        return (nodeIdx >= 0 && nodeIdx < (i32)nodeVisible.size() && !nodeVisible[nodeIdx]) ? 0.0f
                                                                                            : 1.0f;
    };
    ...
    for (i32 mi = 0; mi < (i32)model_.materials.size(); mi++) {
        ...
        for (i32 li = 0; li < (i32)mat.layers.size(); li++) {
            const auto& layer = mat.layers[li];
            const bool anyAnim =
                layer.fresnelColorTracks.isUsed || layer.fresnelAlphaTracks.isUsed ||
                layer.fresnelTeamColorTracks.isUsed || layer.emissiveGainTracks.isUsed;
            if (!anyAnim)
                continue;
    ...
    for (i32 mi = 0; mi < (i32)model_.materials.size(); mi++) {
        ...
        for (i32 li = 0; li < (i32)mat.layers.size(); li++) {
            ...
            for (usize k = 0; k < layer.subTextures.size(); ++k) {
                ...
                switch (k) {
                case 0:
                    slot = FrameState::LayerTexSlot::Diffuse;
                    break;
                case 1:
                    slot = FrameState::LayerTexSlot::Normal;
                    break;
                case 2:
                    slot = FrameState::LayerTexSlot::ORM;
                    break;
                case 3:
                    slot = FrameState::LayerTexSlot::Emissive;
                    break;
                case 4:
                    slot = FrameState::LayerTexSlot::TeamColor;
                    break;
                default:
                    continue;
                }
    ...
    auto worldOf = [&](i32 nodeIdx) -> Matrix44f {
        if (nodeIdx < 0 || nodeIdx >= (i32)fs.boneWorldMatrices.size())
    ...
    const Matrix44f kPE2SpawnFrameRotation = Matrix44f::rotation_z(1.5707963267948966f);
    ...
    const Matrix44f kCornFxSpawnFrameRotation = Matrix44f::rotation_z(1.5707963267948966f);
    ...
    for (i32 i = 0; i < (i32)model_.lights.size(); ++i) {
        ...
        const f32 visibility =
            evalF32(L.visibilityTracks, 1.0f) * gateByBoneAncestors(lightNodeIdx);
...
static EventObjectConfig::Kind DecodeEventKind(std::string_view name) {
    ...
    auto eq = [&](const char* p) { return name[0] == p[0] && name[1] == p[1] && name[2] == p[2]; };
    if (eq("SPN"))
        return EventObjectConfig::Kind::SPN;
    if (eq("SPL"))
        return EventObjectConfig::Kind::SPL;
    if (eq("UBR"))
        return EventObjectConfig::Kind::UBR;
    if (eq("FPT"))
        return EventObjectConfig::Kind::FPT;
    if (eq("SND"))
        return EventObjectConfig::Kind::SND;
    return EventObjectConfig::Kind::Unknown;
}
...
std::vector<EventObjectConfig> MdxModelAdapter::GetEventObjects() {
    ...
    for (const auto& ev : model_.eventObjects) {
        ...
        if (cfg.name.size() >= 4) {
            ...
            while (!tail.empty() && (tail.back() == '\0' || tail.back() == ' '))
...
std::vector<u32> MdxModelAdapter::GetGlobalSequences() {
    return std::vector<u32>(model_.globalSequences.begin(), model_.globalSequences.end());
}
...
std::vector<CameraPreset> MdxModelAdapter::GetCameraPresets() {
    ...
    for (const auto& cam : model_.cameras) {
        ...
        const bool animated = cam.positionTracks.isUsed || cam.targetPositionTracks.isUsed ||
        ...
        if (animated) {
            ...
            cp.animator = [posTracks = std::move(posTracks), tgtTracks = std::move(tgtTracks),
                           rollTracks = std::move(rollTracks), pivot,
                           targetPivot](Vector3f& outPos, Vector3f& outTgt, f32& outRoll,
                ...
                outTgt = {targetPivot.x + tgtDelta.x, targetPivot.y + tgtDelta.y,
                          targetPivot.z + tgtDelta.z};
